...
[[using gnu: const, always_inline, hot]] [[nodiscard]]
constexpr inline std::array<T, 6> quinerpCoeffs(T ratio, T scale = 1.0) noexcept {
...
template <typename T = double>
[[using gnu: const, always_inline, hot]] [[nodiscard]]
constexpr inline std::array<T, 6> quinerpdCoeffs(T ratio, T scale = 1.0) noexcept {
    static_assert(std::is_floating_point_v<T>, "The loaded type must be a floating-point type.");
