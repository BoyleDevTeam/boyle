...
#include "math/functions/piecewise_functions/piecewise_cubic_function1.hpp"
...
template <typename T>
class [[nodiscard]] PiecewiseCubicCurve2 final : public Curve2<PiecewiseCubicCurve2<T>, T> {
    static_assert(std::is_floating_point_v<T>, "The loaded type must be a floating-point type.");
    friend class boost::serialization::access;

  public:
    using BoundaryMode = typename PiecewiseCubicFunction1<Vec2<Vec2Mode::XY, T>, T>::BoundaryMode;

    static constexpr T kDuplicateCriterion{kEpsilon};

    ENABLE_IMPLICIT_CONSTRUCTORS(PiecewiseCubicCurve2);

    [[using gnu: always_inline]] explicit PiecewiseCubicCurve2(
        std::vector<Vec2<Vec2Mode::XY, T>> anchor_points, T s0 = 0.0
    )
        : PiecewiseCubicCurve2(
              std::move(anchor_points), BoundaryMode{2, Vec2<Vec2Mode::XY, T>{0.0, 0.0}},
              BoundaryMode{2, Vec2<Vec2Mode::XY, T>{0.0, 0.0}}, s0
          ) {}

    [[using gnu: flatten, leaf]] explicit PiecewiseCubicCurve2(
        std::vector<Vec2<Vec2Mode::XY, T>> anchor_points, BoundaryMode b0, BoundaryMode bf,
        T s0 = 0.0
    ) {
        ...
        PiecewiseCubicFunction1<Vec2<Vec2Mode::XY, T>, T> temp_vec2_of_s{
        ...
        vec2_of_s_ = PiecewiseCubicFunction1<Vec2<Vec2Mode::XY, T>, T>{
            std::move(arc_lengths), std::move(anchor_points), b0, bf};
    }

    ~PiecewiseCubicCurve2() noexcept override = default;
    ...
    T curvature(T s) const noexcept {
        ...
        if (pos == 0) {
            ...
            const Vec2<Vec2Mode::XY, T> derivative = cuberpd(
                anchor_points[pos - 1], anchor_points[pos], ddys[pos - 1], ddys[pos], ratio, h
            );
            const Vec2<Vec2Mode::XY, T> derivative2 = lerp(ddys[pos - 1], ddys[pos], ratio);
  ...
  private:
    ...
    static T calcArcLength(
        ...
        Vec2<Vec2Mode::XY, T> ddend, T scale
    ) noexcept {
        ...
        for (std::size_t i = 0; i < kNumGaussLegendreKnots; ++i) {
            result += cuberpd(start, end, ddstart, ddend, kGaussLegendreKnots[i], scale).length() *
    ...
    [[using gnu: always_inline]] explicit PiecewiseCubicCurve2(
        PiecewiseCubicFunction1<math::Vec2<Vec2Mode::XY, T>, T> vec2_of_s
    ) noexcept
    ...
    PiecewiseCubicFunction1<Vec2<Vec2Mode::XY, T>, T> vec2_of_s_;
};

template <typename T>
struct isCurve<PiecewiseCubicCurve2<T>> final {
