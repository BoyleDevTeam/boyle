...
#include "math/curves/piecewise_curves/piecewise_cubic_curve2.hpp"
...
TEST_CASE("SemiCircle") {
    ...
    PiecewiseCubicCurve2d semi_circle{anchor_points, b0, bf};

    const std::vector<double>& arc_lengths{semi_circle.arcLengths()};

    CHECK_EQ(semi_circle.minS(), 0.0);
    CHECK_EQ(semi_circle.maxS(), doctest::Approx(M_PI * 2.0).epsilon(kEpsilon));
    ...
    for (double theta : intpl_thetas) {
        ...
        const SlVec2d sl = semi_circle.inverse(point);
        CHECK_EQ(sl.s, doctest::Approx(2.0 * theta).epsilon(kEpsilon));
        CHECK_EQ(sl.l, doctest::Approx(0.0).epsilon(2E-8));
        CHECK_EQ(semi_circle.curvature(sl.s), doctest::Approx(0.5).epsilon(3E-5));
    }

    const std::array<Vec2d, 5> test_points{
        Vec2d{-1.3, 1.8}, Vec2d{-0.2, 1.8}, Vec2d{4.0, 0}, Vec2d{0.8, 1.9}, Vec2d{1.8, 0.3}};
    for (Vec2d point : test_points) {
        CHECK_EQ(
            2.0 - point.length(), doctest::Approx(semi_circle.inverse(point).l).epsilon(kEpsilon)
        );
    }
...
TEST_CASE("LogarithmicSpiral") {
    ...
    const std::size_t num_anchors = 101;
    const std::size_t num_intpls = 50;
    std::vector<double> anchor_thetas = linspace(0.0, M_PI * 8.0, num_anchors);
    std::vector<double> intpl_thetas = linspace(M_PI * 2.0 + 0.1, M_PI * 6.0 - 0.1, num_intpls);
    ...
    PiecewiseCubicCurve2d log_spiral{anchor_points};
    ...
    for (double theta : intpl_thetas) {
        ...
        const SlVec2d sl = log_spiral.inverse(point);
        CHECK_EQ(sl.l, doctest::Approx(0.0).epsilon(2E-4));
    }
...
TEST_CASE("Serialization") {
    ...
    PiecewiseCubicCurve2d other_log_spiral;
