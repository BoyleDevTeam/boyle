...
TEST_CASE("LaneFollow") {
    ...
    const std::vector<double> sample_ss = ::boyle::math::linspace(0.0, 20.0, num_samples);
    Path2d path;
    ...
    SUBCASE("BentLane") {
        ...
        RouteLineCubicOffsetModel route_line_offset_model{sketch_points, std::move(sample_ss)};
        ...
        path = route_line_offset_model.solve().path;
...
TEST_CASE("TrivialScene") {
    ...
    route_line_offset_model.setDdxRange(-1.0, 1.0);
    route_line_offset_model.setDdyRange(-1.0, 1.0);
    RouteLineCubicOffsetModel::Result result = route_line_offset_model.solve();
    const Path2d& path = result.path;
