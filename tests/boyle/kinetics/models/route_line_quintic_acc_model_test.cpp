...
TEST_CASE("CostTest") {
    ...
    RouteLineQuinticAccModel<double> route_line_acc_model{sample_ts};
...
TEST_CASE("TrivialScene") {
    ...
    route_line_acc_model.setSnapCost(0.01);
...
TEST_CASE("HeadwayScene") {
    ...
    if (plot_graph) {
        ...
        std::vector<axes_handle> axes = createAxesHandles(fig);
        axes[0]->plot(
            std::vector<double>(std::from_range_t{}, bound_ts),
            std::vector<double>(std::from_range_t{}, bound_ss), "r-"
        );
...
auto main(int argc, const char* argv[]) -> int {
    cxxopts::Options options(
        "route_line_quintic_acc_model_test", "unit test of RouteLineQuinticAccModel class"
    );
    options.add_options()(
