...
#include "boyle/cvxopm/solvers/bfgs_solver.hpp"
...
TEST_CASE_TEMPLATE("QuadraticTest", T, float, double, long double) {
    ...
    const BfgsSolver<T> bfgs_solver{.settings{.eps_abs{1E-6}}};

    ::boyle::math::pmr::VectorX<T> x0({0.0, -1.0});

    const auto [result, info] = bfgs_solver.solve(dense_problem, std::move(x0));
