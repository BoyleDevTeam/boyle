...
TEST_CASE("LeftSemiCircleTest") {
    ...
    for (std::size_t i{1}; i < kNumAnchors; ++i) {
        ...
        CHECK(semi_circle.tangent(s_exact).identicalTo(tangent, 1E-6));
        CHECK(semi_circle.normal(s_exact).identicalTo(normal, 1E-6));
        CHECK_EQ(semi_circle.curvature(s_exact), doctest::Approx(0.5).epsilon(1E-5));
    }
    ...
    CHECK_EQ(s, doctest::Approx(M_PI / 3.0).epsilon(4E-7));
    CHECK_EQ(l, doctest::Approx(1.0).epsilon(1E-7));
    CHECK(semi_circle(s, l).identicalTo(milestone_points[0], 1E-4));

    std::tie(s, l) = semi_circle.inverse(milestone_points[1]);
    CHECK_EQ(s, doctest::Approx(M_PI * 4.0 / 3.0).epsilon(1E-7));
    CHECK_EQ(l, doctest::Approx(-1.0).epsilon(1E-7));
...
TEST_CASE("RightSemiCircleTest") {
    ...
    auto [s, l] = semi_circle.inverse(milestone_points[0]);
    CHECK_EQ(s, doctest::Approx(M_PI * 5.0 / 3.0).epsilon(1E-7));
    CHECK_EQ(l, doctest::Approx(1.0).epsilon(1E-7));
    CHECK(semi_circle(s, l).identicalTo(milestone_points[0], 1E-4));

    std::tie(s, l) = semi_circle.inverse(milestone_points[1]);
    CHECK_EQ(s, doctest::Approx(M_PI * 2.0 / 3.0).epsilon(1E-7));
    CHECK_EQ(l, doctest::Approx(-1.0).epsilon(1E-7));
    CHECK(semi_circle(s, l).identicalTo(milestone_points[1], 1E-4));
...
TEST_CASE("LogarithmicSpiralTest") {
    ...
    for (std::size_t i{10}; i < kNumAnchors - 9; ++i) {
        ...
        CHECK_EQ(l, doctest::Approx(0.0).epsilon(2E-5));
    }
...
TEST_CASE("LissajousCurveTest") {
    ...
    for (std::size_t i{1}; i < kNumAnchors; ++i) {
        ...
        CHECK_EQ(l, doctest::Approx(0.0).epsilon(6E-4));
    }
