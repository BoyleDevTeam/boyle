...
TEST_CASE("LeftSemiCircleTest") {
    ...
    PiecewiseCubicCurve2d::BoundaryMode b0, bf;
    SUBCASE("Clamped") {
        b0 = PiecewiseCubicCurve2d::BoundaryMode{1, Vec2d{0.0, 1.0}};
        bf = PiecewiseCubicCurve2d::BoundaryMode{1, Vec2d{0.0, -1.0}};
    }
    SUBCASE("Natural") {
        b0 = PiecewiseCubicCurve2d::BoundaryMode{2, Vec2d{-0.5, 0.0}};
        bf = PiecewiseCubicCurve2d::BoundaryMode{2, Vec2d{0.5, 0.0}};
    }

    const PiecewiseCubicCurve2d semi_circle{anchor_points, b0, bf};
    ...
    for (std::size_t i{1}; i < kNumAnchors; ++i) {
        ...
        CHECK_EQ(sl.l, doctest::Approx(0.0).epsilon(1E-5));
        CHECK(semi_circle.tangent(s).approachTo(tangent, 1E-4));
        CHECK(semi_circle.normal(s).approachTo(normal, 1E-4));
        CHECK_EQ(semi_circle.curvature(s), doctest::Approx(0.5).epsilon(1E-3));
    }
    ...
    CHECK_EQ(sl.s, doctest::Approx(M_PI / 3.0).epsilon(1E-4));
    CHECK_EQ(sl.l, doctest::Approx(1.0).epsilon(1E-5));
    CHECK(semi_circle(sl).approachTo(milestone_points[0], 1E-4));

    sl = semi_circle.inverse(milestone_points[1]);
    CHECK_EQ(sl.s, doctest::Approx(4.0 * M_PI / 3.0).epsilon(1E-5));
    CHECK_EQ(sl.l, doctest::Approx(-1.0).epsilon(1E-5));
    CHECK(semi_circle(sl).approachTo(milestone_points[1], 1E-4));
...
TEST_CASE("RightSemiCircleTest") {
    ...
    SlDupletd sl = semi_circle.inverse(milestone_points[0]);
    CHECK_EQ(sl.s, doctest::Approx(M_PI * 5.0 / 3.0).epsilon(1E-4));
    CHECK_EQ(sl.l, doctest::Approx(1.0).epsilon(1E-5));
    CHECK(semi_circle(sl).approachTo(milestone_points[0], 1E-3));

    sl = semi_circle.inverse(milestone_points[1]);
    CHECK_EQ(sl.s, doctest::Approx(M_PI * 2.0 / 3.0).epsilon(1E-5));
    CHECK_EQ(sl.l, doctest::Approx(-1.0).epsilon(1E-5));
    CHECK(semi_circle(sl).approachTo(milestone_points[1], 1E-4));
...
TEST_CASE("LogarithmicSpiralTest") {
    ...
    const PiecewiseCubicCurve2d log_spiral{anchor_points};
    ...
    for (std::size_t i{10}; i < kNumAnchors - 9; ++i) {
        ...
        CHECK_EQ(sl.l, doctest::Approx(0.0).epsilon(2E-3));
    }
...
TEST_CASE("LissajousCurveTest") {
    ...
    const PiecewiseCubicCurve2d lissajous_curve{periodic_tag{}, anchor_points};
    ...
    for (std::size_t i{1}; i < kNumAnchors; ++i) {
        ...
        CHECK_EQ(sl.l, doctest::Approx(0.0).epsilon(7E-4));
    }
...
TEST_CASE("Serialization") {
    ...
    PiecewiseCubicCurve2d other_lissajous_curve;
    ...
    const std::vector<Vec2d>& other_anchor_points{other_lissajous_curve.anchorPoints()};
