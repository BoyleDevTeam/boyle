...
TEST_CASE("SemiCircleTest") {
    ...
    for (std::size_t i{1}; i < kNumAnchors; ++i) {
        const Vec2d point = exact_semi_circle(kStart + (i - 0.5) * kStep);
    ...
    if (plot_graph) {
        ...
        constexpr std::size_t kNumTest{21};
        std::array<double, kNumTest> test_xs, test_ys;
        {
            const double start_s{semi_circle.minS()}, end_s{semi_circle.maxS()};
            const double step_s{(end_s - start_s) / (kNumTest - 1)};
            for (std::size_t i{1}; i < kNumTest; ++i) {
...
TEST_CASE("LogarithmicSpiralTest") {
    ...
    if (plot_graph) {
        ...
        constexpr std::size_t kNumTest{101};
        std::array<double, kNumTest> test_xs, test_ys;
        {
