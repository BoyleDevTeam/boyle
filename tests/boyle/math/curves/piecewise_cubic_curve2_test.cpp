...
TEST_CASE("LeftSemiCircleTest") {
    ...
    for (std::size_t i{1}; i < kNumAnchors; ++i) {
        ...
        CHECK(semi_circle.tangent(s).identicalTo(tangent, 1E-4));
        CHECK(semi_circle.normal(s).identicalTo(normal, 1E-4));
...
TEST_CASE("RightSemiCircleTest") {
    ...
    CHECK_EQ(sl.l, doctest::Approx(1.0).epsilon(1E-5));
    CHECK(semi_circle(sl).identicalTo(milestone_points[0], 1E-3));
