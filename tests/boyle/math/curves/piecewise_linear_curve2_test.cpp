...
TEST_CASE("LeftSemiCircleTest") {
    ...
    const PiecewiseLinearCurve2d semi_circle{anchor_points};
    ...
    for (std::size_t i{1}; i < kNumAnchors; ++i) {
        ...
        CHECK_EQ(sl.s, doctest::Approx(s).epsilon(1E-3));
        CHECK_EQ(sl.l, doctest::Approx(0.0).epsilon(1E-2));
        CHECK(semi_circle.tangent(s).approachTo(tangent, 1E-4));
        CHECK(semi_circle.normal(s).approachTo(normal, 1E-4));
        CHECK(std::isnan(semi_circle.curvature(s)));
    }
    ...
    CHECK_EQ(sl.s, doctest::Approx(M_PI / 3.0).epsilon(2E-2));
    CHECK_EQ(sl.l, doctest::Approx(1.0).epsilon(3E-3));
    CHECK(semi_circle(sl).approachTo(milestone_points[0], 1E-4));

    sl = semi_circle.inverse(milestone_points[1]);
    CHECK_EQ(sl.s, doctest::Approx(M_PI * 4.0 / 3.0).epsilon(1E-2));
    CHECK_EQ(sl.l, doctest::Approx(-1.0).epsilon(6E-3));
...
TEST_CASE("RightSemiCircleTest") {
    ...
    for (std::size_t i{1}; i < kNumAnchors; ++i) {
        ...
        const Vec2d tangent{std::sin(theta), -std::cos(theta)};
        const Vec2d normal{-std::cos(theta), -std::sin(theta)};
        CHECK_EQ(sl.s, doctest::Approx(s).epsilon(1E-2));
        CHECK_EQ(sl.l, doctest::Approx(0.0).epsilon(1E-2));
        CHECK(semi_circle.tangent(s).approachTo(tangent, 1E-4));
        CHECK(semi_circle.normal(s).approachTo(normal, 1E-4));
        CHECK(std::isnan(semi_circle.curvature(s)));
    ...
    SlDupletd sl = semi_circle.inverse(milestone_points[0]);
    CHECK_EQ(sl.s, doctest::Approx(M_PI * 5.0 / 3.0).epsilon(2E-1));
    CHECK_EQ(sl.l, doctest::Approx(1.0).epsilon(3E-3));
    CHECK(semi_circle(sl).approachTo(milestone_points[0], 1E-4));

    sl = semi_circle.inverse(milestone_points[1]);
    CHECK_EQ(sl.s, doctest::Approx(M_PI * 2.0 / 3.0).epsilon(3E-2));
    CHECK_EQ(sl.l, doctest::Approx(-1.0).epsilon(6E-3));
    CHECK(semi_circle(sl).approachTo(milestone_points[1], 1E-4));
...
TEST_CASE("LogarithmicSpiralTest") {
    ...
    const PiecewiseLinearCurve2d log_spiral{anchor_points};
    ...
    for (std::size_t i{5}; i < kNumAnchors - 4; ++i) {
        ...
        CHECK_EQ(sl.l, doctest::Approx(0.0).epsilon(1));
    }
...
TEST_CASE("LissajousCurveTest") {
    ...
    const PiecewiseLinearCurve2d lissajous_curve{anchor_points};
...
TEST_CASE("Serialization") {
    ...
    PiecewiseLinearCurve2d other_lissajous_curve;
