...
TEST_CASE("LeftSemiCircleTest") {
    ...
    const PiecewiseLinearCurve<Vec2d> semi_circle{anchor_points};

    CHECK_EQ(semi_circle.minS(), 0.0);
    CHECK_EQ(semi_circle.maxS(), doctest::Approx(2.0 * M_PI).epsilon(1E-2));
    ...
    for (std::size_t i{1}; i < kNumAnchors; ++i) {
        ...
        CHECK(std::isnan(semi_circle.curvature(s_exact)));
    }
    ...
    CHECK_EQ(s, doctest::Approx(M_PI / 3.0).epsilon(2E-2));
    CHECK_EQ(l, doctest::Approx(1.0).epsilon(3E-3));
    CHECK(semi_circle(s, l).identicalTo(milestone_points[0], 1E-4));

    std::tie(s, l) = semi_circle.inverse(milestone_points[1]);
    CHECK_EQ(s, doctest::Approx(M_PI * 4.0 / 3.0).epsilon(1E-2));
...
TEST_CASE("RightSemiCircleTest") {
    ...
    for (std::size_t i{1}; i < kNumAnchors; ++i) {
        ...
        const Vec2d tangent{std::sin(theta), -std::cos(theta)};
        const Vec2d normal{-std::cos(theta), -std::sin(theta)};
        CHECK_EQ(s, doctest::Approx(s_exact).epsilon(1E-2));
        CHECK_EQ(l, doctest::Approx(0.0).epsilon(1E-2));
        CHECK(semi_circle.tangent(s_exact).identicalTo(tangent, 1E-4));
        CHECK(semi_circle.normal(s_exact).identicalTo(normal, 1E-4));
        CHECK(std::isnan(semi_circle.curvature(s_exact)));
    ...
    auto [s, l] = semi_circle.inverse(milestone_points[0]);
    CHECK_EQ(s, doctest::Approx(M_PI * 5.0 / 3.0).epsilon(2E-1));
    CHECK_EQ(l, doctest::Approx(1.0).epsilon(3E-3));
    CHECK(semi_circle(s, l).identicalTo(milestone_points[0], 1E-4));

    std::tie(s, l) = semi_circle.inverse(milestone_points[1]);
    CHECK_EQ(s, doctest::Approx(M_PI * 2.0 / 3.0).epsilon(3E-2));
    CHECK_EQ(l, doctest::Approx(-1.0).epsilon(6E-3));
    CHECK(semi_circle(s, l).identicalTo(milestone_points[1], 1E-4));
...
TEST_CASE("LogarithmicSpiralTest") {
    ...
    const PiecewiseLinearCurve<Vec2d> log_spiral{anchor_points};

    for (std::size_t i{5}; i < kNumAnchors - 4; ++i) {
        const Vec2d point = exact_log_spiral(kStart + (i - 0.5) * kStep);
        const auto [s, l] = log_spiral.inverse(point);
        CHECK_EQ(l, doctest::Approx(0.0).epsilon(1));
    }
...
TEST_CASE("LissajousCurveTest") {
    ...
    const PiecewiseLinearCurve<Vec2d> lissajous_curve{anchor_points};
...
TEST_CASE("Serialization") {
    ...
    PiecewiseLinearCurve<Vec2d> other_lissajous_curve;
...
auto main(int argc, const char* argv[]) -> int {
    cxxopts::Options options(
        "piecewise_linear_curve2_test", "unit test of PiecewiseLinearCurve2 class"
    );
    options.add_options()(
