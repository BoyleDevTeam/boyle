/**
 * @file piecewise_linear_curve3_test.cpp
...
TEST_CASE("LeftHelixTest") {
    ...
    const PiecewiseLinearCurve3d helix{anchor_points};
    CHECK_EQ(helix.minS(), 0.0);
    CHECK_EQ(helix.maxS(), doctest::Approx(4.0 * M_PI * std::sqrt(5.0)).epsilon(1E-3));
    ...
    for (std::size_t i{1}; i < kNumAnchors; ++i) {
        ...
        CHECK_EQ(slv.s, doctest::Approx(s).epsilon(1E-3));
        CHECK_EQ(slv.l, doctest::Approx(0.0).epsilon(1E-2));
        CHECK_EQ(slv.v, doctest::Approx(0.0).epsilon(1E-2));
        CHECK(helix.tangent(s).identicalTo(tangent, 1E-3));
        CHECK(helix.normal(s).identicalTo(normal, 1E-1));
        CHECK(helix.binormal(s).identicalTo(binormal, 1E-1));
        CHECK(std::isnan(helix.curvature(s)));
        CHECK(std::isnan(helix.torsion(s)));
    }
    ...
    CHECK_EQ(slv.s, doctest::Approx(M_PI / 6.0 * std::sqrt(5.0)).epsilon(2E-2));
    CHECK_EQ(slv.l, doctest::Approx(1.0).epsilon(6E-3));
    CHECK_EQ(slv.v, doctest::Approx(0.0).epsilon(3E-2));
    CHECK(helix(slv).identicalTo(milestone_points[0], 1E-4));

    slv = helix.inverse(milestone_points[1]);
    CHECK_EQ(slv.s, doctest::Approx(M_PI * 2.0 / 3.0 * std::sqrt(5.0)).epsilon(1E-2));
    CHECK_EQ(slv.l, doctest::Approx(-1.0).epsilon(6E-3));
    CHECK_EQ(slv.v, doctest::Approx(0.0).epsilon(3E-2));
    CHECK(helix(slv).identicalTo(milestone_points[1], 1E-4));
...
TEST_CASE("RightHelixTest") {
    ...
    const PiecewiseLinearCurve3d helix{anchor_points};
    CHECK_EQ(helix.minS(), 0.0);
    CHECK_EQ(helix.maxS(), doctest::Approx(4.0 * M_PI * std::sqrt(5.0)).epsilon(1E-2));
    ...
    SlvTripletd slv = helix.inverse(milestone_points[0]);
    CHECK_EQ(slv.s, doctest::Approx(M_PI * 11.0 / 6.0 * std::sqrt(5.0)).epsilon(1E-2));
    CHECK_EQ(slv.l, doctest::Approx(1.0).epsilon(6E-3));
    CHECK_EQ(slv.v, doctest::Approx(0.0).epsilon(3E-2));
    CHECK(helix(slv).identicalTo(milestone_points[0], 1E-4));

    slv = helix.inverse(milestone_points[1]);
    CHECK_EQ(slv.s, doctest::Approx(M_PI * 4.0 / 3.0 * std::sqrt(5.0)).epsilon(1E-2));
    CHECK_EQ(slv.l, doctest::Approx(-1.0).epsilon(6E-3));
    CHECK_EQ(slv.v, doctest::Approx(0.0).epsilon(5E-2));
    CHECK(helix(slv).identicalTo(milestone_points[1], 1E-4));
...
TEST_CASE("LissajousCurveTest") {
    ...
    const PiecewiseLinearCurve3d lissajous_curve{anchor_points};
    ...
    for (std::size_t i{1}; i < kNumAnchors; ++i) {
        ...
        CHECK_EQ(slv.l, doctest::Approx(0.0).epsilon(5E-2));
        CHECK_EQ(slv.v, doctest::Approx(0.0).epsilon(1E-2));
    }
...
TEST_CASE("Serialization") {
    ...
    PiecewiseLinearCurve3d other_lissajous_curve;
