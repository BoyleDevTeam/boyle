...
TEST_CASE("LeftHelixTest") {
    ...
    for (std::size_t i{1}; i < kNumAnchors; ++i) {
        ...
        CHECK(helix.tangent(s).approachTo(tangent, 1E-3));
        CHECK(helix.normal(s).approachTo(normal, 1E-1));
        CHECK(helix.binormal(s).approachTo(binormal, 1E-1));
