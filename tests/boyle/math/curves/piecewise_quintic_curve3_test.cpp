/**
 * @file piecewise_quintic_curve3_test.cpp
...
TEST_CASE("LeftHelixTest") {
    ...
    std::array<PiecewiseQuinticCurve3d::BoundaryMode, 2> b0, bf;
    SUBCASE("Clamped") {
        b0 = std::array<PiecewiseQuinticCurve3d::BoundaryMode, 2>{
            PiecewiseQuinticCurve3d::BoundaryMode{
                1, Vec3d{0.0, 2.0 / std::sqrt(5.0), 1.0 / std::sqrt(5.0)}
            },
            PiecewiseQuinticCurve3d::BoundaryMode{3, Vec3d{0.0, -0.4 / std::sqrt(5.0), 0.0}}
        };
        bf = std::array<PiecewiseQuinticCurve3d::BoundaryMode, 2>{
            PiecewiseQuinticCurve3d::BoundaryMode{
                1, Vec3d{0.0, 2.0 / std::sqrt(5.0), 1.0 / std::sqrt(5.0)}
            },
            PiecewiseQuinticCurve3d::BoundaryMode{3, Vec3d{0.0, -0.4 / std::sqrt(5.0), 0.0}}
    ...
    SUBCASE("Natural") {
        b0 = std::array<PiecewiseQuinticCurve3d::BoundaryMode, 2>{
            PiecewiseQuinticCurve3d::BoundaryMode{2, Vec3d{-0.4, 0.0, 0.0}},
            PiecewiseQuinticCurve3d::BoundaryMode{4, Vec3d{0.08, 0.0, 0.0}}
        };
        bf = std::array<PiecewiseQuinticCurve3d::BoundaryMode, 2>{
            PiecewiseQuinticCurve3d::BoundaryMode{2, Vec3d{-0.4, 0.0, 0.0}},
            PiecewiseQuinticCurve3d::BoundaryMode{4, Vec3d{0.08, 0.0, 0.0}}
    ...
    const PiecewiseQuinticCurve3d helix{anchor_points, b0, bf};
    CHECK_EQ(helix.minS(), 0.0);
    CHECK_EQ(helix.maxS(), doctest::Approx(4.0 * M_PI * std::sqrt(5.0)).epsilon(1E-7));
    ...
    for (std::size_t i{1}; i < kNumAnchors; ++i) {
        ...
        CHECK_EQ(slv.l, doctest::Approx(0.0).epsilon(1E-7));
        CHECK_EQ(slv.v, doctest::Approx(0.0).epsilon(1E-7));
        CHECK(helix.tangent(s).approachTo(tangent, 1E-6));
        CHECK(helix.normal(s).approachTo(normal, 1E-6));
        CHECK(helix.binormal(s).approachTo(binormal, 1E-6));
        CHECK_EQ(helix.curvature(s), doctest::Approx(2.0 / 5.0).epsilon(2E-6));
        CHECK_EQ(helix.torsion(s), doctest::Approx(1.0 / 5.0).epsilon(2E-6));
    }
    ...
    CHECK_EQ(slv.s, doctest::Approx(M_PI / 6.0 * std::sqrt(5.0)).epsilon(3E-7));
    CHECK_EQ(slv.l, doctest::Approx(1.0).epsilon(1E-8));
    CHECK_EQ(slv.v, doctest::Approx(0.0).epsilon(3E-7));
    CHECK(helix(slv).approachTo(milestone_points[0], 3E-7));

    slv = helix.inverse(milestone_points[1]);
    CHECK_EQ(slv.s, doctest::Approx(M_PI * 2.0 / 3.0 * std::sqrt(5.0)).epsilon(1E-7));
    CHECK_EQ(slv.l, doctest::Approx(-1.0).epsilon(1E-8));
    CHECK_EQ(slv.v, doctest::Approx(0.0).epsilon(1E-7));
    CHECK(helix(slv).approachTo(milestone_points[1], 1E-7));
...
TEST_CASE("RightHelixTest") {
    ...
    SlvTripletd slv = helix.inverse(milestone_points[0]);
    CHECK_EQ(slv.s, doctest::Approx(M_PI * 11.0 / 6.0 * std::sqrt(5.0)).epsilon(3E-7));
    CHECK_EQ(slv.l, doctest::Approx(1.0).epsilon(1E-8));
    CHECK_EQ(slv.v, doctest::Approx(0.0).epsilon(3E-7));
    CHECK(helix(slv).approachTo(milestone_points[0], 3E-7));

    slv = helix.inverse(milestone_points[1]);
    CHECK_EQ(slv.s, doctest::Approx(M_PI * 4.0 / 3.0 * std::sqrt(5.0)).epsilon(1E-7));
    CHECK_EQ(slv.l, doctest::Approx(-1.0).epsilon(1E-8));
    CHECK_EQ(slv.v, doctest::Approx(0.0).epsilon(1E-7));
    CHECK(helix(slv).approachTo(milestone_points[1], 1E-7));
...
TEST_CASE("LissajousCurveTest") {
    ...
    const PiecewiseQuinticCurve3d lissajous_curve{periodic_tag{}, anchor_points};

    const std::vector<double>& arc_lengths{lissajous_curve.arcLengths()};
...
TEST_CASE("Serialization") {
    ...
    PiecewiseQuinticCurve3d other_lissajous_curve;

    std::istringstream iss(oss.str());
    boost::archive::binary_iarchive ia(iss);
    ia >> other_lissajous_curve;

    const std::vector<double>& arc_length{lissajous_curve.arcLengths()};
    const std::vector<double>& other_arc_length{other_lissajous_curve.arcLengths()};
    const std::vector<Vec3d>& other_anchor_points{other_lissajous_curve.anchorPoints()};
