...
TEST_CASE("LeftHelixTest") {
    ...
    std::array<BoundaryMode<Vec3d>, 2> b0, bf;
    SUBCASE("Clamped") {
        b0 = std::array<BoundaryMode<Vec3d>, 2>{
            BoundaryMode<Vec3d>{
                .order = 1, .derivative = Vec3d{0.0, 2.0 / std::sqrt(5.0), 1.0 / std::sqrt(5.0)}
            },
            BoundaryMode<Vec3d>{.order = 3, .derivative = Vec3d{0.0, -0.4 / std::sqrt(5.0), 0.0}}
        };
        bf = std::array<BoundaryMode<Vec3d>, 2>{
            BoundaryMode<Vec3d>{
                .order = 1, .derivative = Vec3d{0.0, 2.0 / std::sqrt(5.0), 1.0 / std::sqrt(5.0)}
            },
            BoundaryMode<Vec3d>{.order = 3, .derivative = Vec3d{0.0, -0.4 / std::sqrt(5.0), 0.0}}
    ...
    SUBCASE("Natural") {
        b0 = std::array<BoundaryMode<Vec3d>, 2>{
            BoundaryMode<Vec3d>{.order = 2, .derivative = Vec3d{-0.4, 0.0, 0.0}},
            BoundaryMode<Vec3d>{.order = 4, .derivative = Vec3d{0.08, 0.0, 0.0}}
        };
        bf = std::array<BoundaryMode<Vec3d>, 2>{
            BoundaryMode<Vec3d>{.order = 2, .derivative = Vec3d{-0.4, 0.0, 0.0}},
            BoundaryMode<Vec3d>{.order = 4, .derivative = Vec3d{0.08, 0.0, 0.0}}
    ...
    const PiecewiseQuinticCurve<Vec3d> helix{anchor_points, b0, bf};
    ...
    for (std::size_t i{1}; i < kNumAnchors; ++i) {
        ...
        const double s_exact{theta * std::sqrt(5.0)};
        const Vec3d point = exact_helix(theta);
        const auto [s, l, v] = helix.inverse(point);
        const Vec3d tangent{
        ...
        CHECK_EQ(l, doctest::Approx(0.0).epsilon(1E-7));
        CHECK_EQ(v, doctest::Approx(0.0).epsilon(1E-7));
        CHECK(helix.tangent(s).identicalTo(tangent, 1E-6));
        CHECK(helix.normal(s).identicalTo(normal, 1E-6));
        CHECK(helix.binormal(s).identicalTo(binormal, 1E-6));
    ...
    double s, l, v;
    std::tie(s, l, v) = helix.inverse(milestone_points[0]);
    CHECK_EQ(s, doctest::Approx(M_PI / 6.0 * std::sqrt(5.0)).epsilon(3E-7));
    CHECK_EQ(l, doctest::Approx(1.0).epsilon(1E-8));
    CHECK_EQ(v, doctest::Approx(0.0).epsilon(3E-7));
    CHECK(helix(s, l, v).identicalTo(milestone_points[0], 3E-7));

    std::tie(s, l, v) = helix.inverse(milestone_points[1]);
    CHECK_EQ(s, doctest::Approx(M_PI * 2.0 / 3.0 * std::sqrt(5.0)).epsilon(1E-7));
    CHECK_EQ(l, doctest::Approx(-1.0).epsilon(1E-8));
    CHECK_EQ(v, doctest::Approx(0.0).epsilon(1E-7));
    CHECK(helix(s, l, v).identicalTo(milestone_points[1], 1E-7));
...
TEST_CASE("RightHelixTest") {
    ...
    double s, l, v;
    std::tie(s, l, v) = helix.inverse(milestone_points[0]);
    CHECK_EQ(s, doctest::Approx(M_PI * 11.0 / 6.0 * std::sqrt(5.0)).epsilon(3E-7));
    CHECK_EQ(l, doctest::Approx(1.0).epsilon(1E-8));
    CHECK_EQ(v, doctest::Approx(0.0).epsilon(3E-7));
    CHECK(helix(s, l, v).identicalTo(milestone_points[0], 3E-7));

    std::tie(s, l, v) = helix.inverse(milestone_points[1]);
    CHECK_EQ(s, doctest::Approx(M_PI * 4.0 / 3.0 * std::sqrt(5.0)).epsilon(1E-7));
    CHECK_EQ(l, doctest::Approx(-1.0).epsilon(1E-8));
    CHECK_EQ(v, doctest::Approx(0.0).epsilon(1E-7));
    CHECK(helix(s, l, v).identicalTo(milestone_points[1], 1E-7));
...
TEST_CASE("LissajousCurveTest") {
    ...
    const PiecewiseQuinticCurve<Vec3d> lissajous_curve{periodic_tag{}, anchor_points};

    const auto& arc_lengths{lissajous_curve.arcLengths()};

    for (std::size_t i{1}; i < kNumAnchors; ++i) {
        const Vec3d point = exact_lissajous_curve(kStart + (i - 0.5) * kStep);
        const auto [s, l, v] = lissajous_curve.inverse(point, arc_lengths[i - 1], arc_lengths[i]);
        CHECK_EQ(l, doctest::Approx(0.0).epsilon(3E-2));
        CHECK_EQ(v, doctest::Approx(0.0).epsilon(7E-3));
    }
...
TEST_CASE("Serialization") {
    ...
    PiecewiseQuinticCurve<Vec3d> other_lissajous_curve;
