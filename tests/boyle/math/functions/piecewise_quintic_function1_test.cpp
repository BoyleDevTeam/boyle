...
TEST_CASE("QuinticPolynomial") {
    ...
    const PiecewiseQuinticFunction1d func{ts, ys, {b01, b02}, {bf1, bf2}};
    ...
    if (plot_graph) {
        ...
        axes[4]->plot(
            std::vector<double>{func.ts().begin(), func.ts().end()},
            transform(std::vector<double>{func.ts().begin(), func.ts().end()}, exact_d4func), "."
        );
        auto d4func = [&func](double t) -> double { return func.derivative(t, 4); };
        axes[4]->plot(plot_ts, transform(plot_ts, d4func), "-");
        axes[5]->plot(
            std::vector<double>{func.ts().begin(), func.ts().end()},
            transform(std::vector<double>{func.ts().begin(), func.ts().end()}, exact_d5func), "."
        );
...
TEST_CASE("SineTest") {
    ...
    const PiecewiseQuinticFunction1d func{periodic_tag{}, ts, ys};
...
TEST_CASE("Serialization") {
    ...
    const PiecewiseQuinticFunction1d func{ts, ys};
