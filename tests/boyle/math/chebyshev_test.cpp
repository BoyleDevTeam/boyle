...
TEST_CASE("SinFunc") {
    auto exact_sin = [](double t) -> double { return std::sin(t); };
    const double lower_bound{0.0}, upper_bound{M_PI * 2.0};
    const int num_total{10};

    const Chebyshev<double, int> chebyshev_sin{exact_sin, lower_bound, upper_bound, num_total};
    const Chebyshev chebyshev_dsin = chebyshev_sin.derivative();
    const Chebyshev chebyshev_isin = chebyshev_sin.integral();

    CHECK_EQ(chebyshev_sin.num_total(), num_total);
    CHECK_EQ(chebyshev_sin.num_trunc(), num_total);
    CHECK_EQ(chebyshev_sin.lower_bound(), lower_bound);
    CHECK_EQ(chebyshev_sin.upper_bound(), upper_bound);

    CHECK_EQ(chebyshev_dsin.num_total(), num_total);
    CHECK_EQ(chebyshev_dsin.num_trunc(), num_total);
    CHECK_EQ(chebyshev_dsin.lower_bound(), lower_bound);
    CHECK_EQ(chebyshev_dsin.upper_bound(), upper_bound);

    CHECK_EQ(chebyshev_isin.num_total(), num_total);
    CHECK_EQ(chebyshev_isin.num_trunc(), num_total);
...
TEST_CASE("Serialization") {
    ...
    Chebyshev other_chebyshev_sin;

    std::istringstream iss(oss.str());
    boost::archive::binary_iarchive ia(iss);
    ia >> other_chebyshev_sin;

    CHECK_EQ(other_chebyshev_sin.num_total(), chebyshev_sin.num_total());
    ...
    for (std::size_t i{0}; i < other_chebyshev_sin.num_total(); ++i) {
        CHECK_EQ(other_chebyshev_sin.coeffs()[i], chebyshev_sin.coeffs()[i]);
    }
}
