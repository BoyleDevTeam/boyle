...
class AlignedMemoryResource final : public std::pmr::memory_resource {
  ...
  protected:
    ...
    auto do_allocate(std::size_t bytes, std::size_t alignment) -> void* override {
        void* ptr{nullptr};
        std::size_t slice{std::max(m_alignment, alignment)};
#ifdef _MSC_VER
        ptr = _aligned_malloc(bytes + slice - bytes % slice, std::max(m_alignment, alignment));
#else
        ptr = std::aligned_alloc(slice, bytes + slice - bytes % slice);
#endif
