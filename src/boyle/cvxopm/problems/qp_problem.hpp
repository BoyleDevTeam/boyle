...
class QpProblem final {
  ...
  public:
    ...
    template <typename OtherAlloc = ::boyle::common::AlignedAllocator<value_type, 32>>
    [[using gnu: always_inline, hot]]
    auto addRampCostTerm(
        const boyle::math::DokVector<value_type, index_type, OtherAlloc>& c_constrain_vec,
        value_type offset, value_type linear_coeff, value_type quadratic_coeff = 0.0
    ) noexcept -> void {
        const size_type num_vars{num_variables()}, num_cons{num_constraints()};
        boyle::math::DokVector<value_type, index_type, allocator_type> constrain_vec(
            num_vars + 1, get_allocator()
        );
        for (const auto& [i, value] : c_constrain_vec.dictionary()) {
            constrain_vec.updateCoeff(i, value);
        }
        ...
        m_constrain_matrix.updateRow(
            num_cons, boyle::math::DokVector<value_type, index_type, allocator_type>{
                          {{num_vars, 1.0}}, get_allocator()
                      }
        );
    ...
    auto addConstrainTerm(
        const boyle::math::DokVector<value_type, index_type, OtherAlloc>& constrain_vec,
