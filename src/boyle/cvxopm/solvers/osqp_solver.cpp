...
auto OsqpSolver<OSQPFloat, OSQPInt>::solve(
    ...
    const ::boyle::math::CscMatrix<OSQPFloat, OSQPInt> objective_matrix{
        qp_problem.m_objective_matrix
    };
    const ::boyle::math::CscMatrix<OSQPFloat, OSQPInt> constrain_matrix{
    ...
    ::boyle::cvxopm::Result<OSQPFloat> result{
        .prim_vars{
            ::boyle::math::VectorView<OSQPFloat>(solver->solution->x, num_vars, 1),
            settings.memory_resource.get()
        },
        .dual_vars{
            ::boyle::math::VectorView<OSQPFloat>(solver->solution->y, num_cons, 1),
            settings.memory_resource.get()
        },
        .prim_inf_cert{
            ::boyle::math::VectorView<OSQPFloat>(solver->solution->prim_inf_cert, num_vars, 1),
            settings.memory_resource.get()
        },
        .dual_inf_cert{
            ::boyle::math::VectorView<OSQPFloat>(solver->solution->dual_inf_cert, num_cons, 1),
            settings.memory_resource.get()
        },
    };
