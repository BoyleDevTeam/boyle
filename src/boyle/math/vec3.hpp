...
class Vec3 final : public Triplet<T> {
  ...
  public:
    ...
    constexpr operator Vec3<U>() noexcept {
    ...
    [[using gnu: pure, always_inline, leaf]]
    constexpr auto operator==(Vec3 other) const noexcept -> bool {
        return x() == other.x() && y() == other.y() && z() == other.z();
    }

    [[using gnu: pure, always_inline]]
    constexpr auto operator+(Vec3 other) const noexcept -> Vec3 {
...
template <std::floating_point T>
[[using gnu: pure, always_inline]]
inline constexpr auto operator*(Arithmetic auto factor, Vec3<T> obj) noexcept -> Vec3<T> {
    return obj * factor;
}
