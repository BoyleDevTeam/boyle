...
struct Vec3 final {
    ...
    [[using gnu: pure, always_inline]]
    constexpr auto crossNorm(InstanceOfTemplate<Vec3> auto obj) const noexcept -> value_type {
        return cross(obj).norm();
    }
    [[using gnu: pure, always_inline]]
    ...
    [[using gnu: pure, always_inline]]
    constexpr auto approachTo(InstanceOfTemplate<Vec3> auto obj, value_type tol = 1E-12)
        const noexcept -> bool {
        return this->euclideanSqrTo(obj) < tol;
    }
    [[using gnu: pure, always_inline]]
