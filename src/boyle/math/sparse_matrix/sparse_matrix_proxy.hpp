...
PRO_DEF_MEM_DISPATCH(m_nrows, nrows);
PRO_DEF_MEM_DISPATCH(m_ncols, ncols);
PRO_DEF_MEM_DISPATCH(m_nnzs, nnzs);
...
PRO_DEF_MEM_DISPATCH(m_coeff, coeff);
...
class [[nodiscard]] SparseMatrixFacader final : public pro::facade_builder
    ::add_convention<m_nrows, auto() noexcept -> std::size_t>
    ::add_convention<m_ncols, auto() noexcept -> std::size_t>
    ::add_convention<m_nnzs, auto() noexcept -> std::size_t>
    ...
    ::template add_convention<m_coeff, auto(Index, Index) noexcept -> Scalar>
...
using SparseMatrixProxy = pro::proxy<detail::SparseMatrixFacader<Scalar, Index>>;
...
inline auto makeSparseMatrixProxy(T sparse_matrix)
    ...
    return pro::make_proxy<
        detail::SparseMatrixFacader<typename T::value_type, typename T::index_type>>(
