...
class [[nodiscard]] CsrMatrix final {
  ...
  public:
    ...
    [[using gnu: flatten, leaf]] CsrMatrix(const DokMatrix<Scalar, Index>& dok_matrix) noexcept
        ...
        const std::map<IndexPair<Index>, Scalar, IndexPairRowMajorCompare<Index>>
            row_major_dictionary{dok_matrix.dictionary().cbegin(), dok_matrix.dictionary().cend()};
