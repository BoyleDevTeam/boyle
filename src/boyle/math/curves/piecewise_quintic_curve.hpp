...
template <VecArithmetic T, typename Alloc = ::boyle::common::AlignedAllocator<T, alignof(T)>>
...
class PiecewiseQuinticCurve final {
  ...
  public:
    ...
    explicit PiecewiseQuinticCurve(
        std::span<const value_type> anchor_points, param_type s0 = 0.0,
    ...
    [[using gnu: ]]
    explicit PiecewiseQuinticCurve(
        std::span<const value_type> anchor_points, std::array<BoundaryMode<value_type>, 2> b0,
        ...
        requires Vec2Arithmetic<value_type>
    {
        ...
        const size_type pos =
            nearestUpperElement(
                anchor_points.cbegin() + istart, anchor_points.cbegin() + iend, point
            ) -
            anchor_points.cbegin();
        if (pos == istart) {
...
template <typename Alloc = ::boyle::common::AlignedAllocator<Vec2s, alignof(Vec2s)>>
using PiecewiseQuinticCurve2s = PiecewiseQuinticCurve<Vec2s, Alloc>;

template <typename Alloc = ::boyle::common::AlignedAllocator<Vec2d, alignof(Vec2d)>>
using PiecewiseQuinticCurve2d = PiecewiseQuinticCurve<Vec2d, Alloc>;

template <typename Alloc = ::boyle::common::AlignedAllocator<Vec3s, alignof(Vec3s)>>
using PiecewiseQuinticCurve3s = PiecewiseQuinticCurve<Vec3s, Alloc>;

template <typename Alloc = ::boyle::common::AlignedAllocator<Vec3d, alignof(Vec3d)>>
