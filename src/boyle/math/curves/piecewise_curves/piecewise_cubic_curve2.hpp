...
class [[nodiscard]] PiecewiseCubicCurve2 final {
  ...
  public:
    ...
    [[using gnu: always_inline]] explicit PiecewiseCubicCurve2(
        std::vector<Vec2<T>> anchor_points, T s0 = 0.0
    )
    ...
    [[using gnu: pure, always_inline]]
    auto
    operator()(T s) const noexcept -> Vec2<T> {
        return eval(s);
    }

    [[using gnu: pure, always_inline]]
    auto
    operator()(T s, T l) const noexcept -> Vec2<T> {
        return eval(s, l);
    }

    [[using gnu: pure, always_inline]]
    auto
    operator()(SlDuplet<T> sl) const noexcept -> Vec2<T> {
        return eval(sl);
    }
  ...
  private:
    ...
    [[using gnu: always_inline]] explicit PiecewiseCubicCurve2(
        PiecewiseCubicFunction1<math::Vec2<T>, T> vec2_of_s
    ) noexcept
