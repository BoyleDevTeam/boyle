...
class Vec2 final : public Duplet<T> {
  ...
  public:
    ...
    [[using gnu: pure, always_inline, leaf]]
    constexpr auto operator==(Vec2 other) const noexcept -> bool {
    ...
    [[using gnu: pure, always_inline]]
    constexpr auto operator+(Vec2 other) const noexcept -> Vec2 {
...
template <std::floating_point T>
[[using gnu: pure, always_inline]]
inline constexpr auto operator*(Arithmetic auto factor, Vec2<T> obj) noexcept -> Vec2<T> {
    return obj * factor;
}
