  ...
  public:
    ...
    Array(const InstanceOfTemplate<Array> auto& other) noexcept
        ...
        std::transform(
            ...
            [](std::remove_cvref_t<decltype(other)>::value_type a) noexcept -> value_type {
...
#endif
        std::transform(cbegin(), cend(), begin(), [fac](value_type x) noexcept -> value_type {
            return x * fac;
        });
        return *this;
    }
    ...
    [[using gnu: pure, always_inline]]
    auto normSqr() const noexcept -> value_type {
        return std::transform_reduce(cbegin(), cend(), cbegin(), 0.0);
    }

    [[using gnu: pure, always_inline]]
    auto normalized() const noexcept(!BOYLE_CHECK_PARAMS) -> Array {
    ...
    [[using gnu: pure, always_inline]]
    auto euclideanSqrTo(const InstanceOfTemplate<Array> auto& obj) const
        noexcept(!BOYLE_CHECK_PARAMS) -> value_type {
#if BOYLE_CHECK_PARAMS == 1
        if (size() != obj.size()) [[unlikely]] {
            throw std::invalid_argument(fmt::format(
...
#endif
        return this->operator-(obj).normSqr();
    }

    [[using gnu: pure, always_inline]]
    auto approachTo(const InstanceOfTemplate<Array> auto& obj, value_type tol = 1E-12) const
        noexcept(!BOYLE_CHECK_PARAMS) -> bool {
#if BOYLE_CHECK_PARAMS == 1
        if (size() != obj.size()) [[unlikely]] {
            throw std::invalid_argument(fmt::format(
        ...
        return std::transform_reduce(
                   ...
                   [](value_type a, value_type b) noexcept -> value_type { return std::max(a, b); },
                   [](value_type a, value_type b) noexcept -> value_type {
    ...
    [[using gnu: pure, always_inline]]
    auto orthogonalTo(const InstanceOfTemplate<Array> auto& obj, value_type tol = 1E-8) const
        noexcept(!BOYLE_CHECK_PARAMS) -> bool {
#if BOYLE_CHECK_PARAMS == 1
        if (size() != obj.size()) [[unlikely]] {
            throw std::invalid_argument(fmt::format(
...
inline auto operator*(Arithmetic auto fac, InstanceOfTemplate<Array> auto&& obj) noexcept
    -> std::add_rvalue_reference_t<decltype(obj)> {
...
inline auto abs(const boyle::math::InstanceOfTemplate<boyle::math::Array> auto& obj) noexcept ->
    typename std::remove_cvref_t<decltype(obj)>::value_type {
    return obj.norm();
}
