...
template <GeneralArithmetic T, typename Alloc = std::allocator<T>>
...
[[using gnu: pure, always_inline]]
inline auto linspace(
    T start, T end, std::size_t num, bool endpoint = true,
    const std::pmr::polymorphic_allocator<T>& alloc = {}
) noexcept -> std::pmr::vector<T> {
    std::pmr::vector<T> result(num, alloc);
    linspace(start, end, num, result.begin(), endpoint);
    return result;
}
