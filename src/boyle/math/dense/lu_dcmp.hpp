...
class [[nodiscard]] LuDcmp final {
  public:
    ...
    explicit LuDcmp(matrix_type matrix) : m_lu{std::move(matrix)} {
    ...
    auto solve() const -> matrix_type {
        ...
        if (info != 0) {
            throw std::runtime_error("LAPACKE_xgetrs failed with info = " + std::to_string(info));
        }
        return b;
#else
        const size_type n{m_lu.ncols()};
