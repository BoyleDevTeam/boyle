...
class QrDcmp final {
  public:
    ...
    explicit QrDcmp(matrix_type matrix) : m_qr{std::move(matrix)}, m_tau(m_qr.nrows()) {
        ...
        if (info != 0) {
            throw std::runtime_error("QrDcmp: LAPACKE_sgeqrf_work failed");
        }
#else
    ...
    auto solve() const -> matrix_type {
        ...
        if (info != 0) {
            throw std::runtime_error("QrDcmp: LAPACKE_sormqr_work failed");
        }
        if constexpr (std::is_same_v<value_type, float>) {
        ...
        if (info != 0) {
            throw std::runtime_error("QrDcmp: LAPACKE_strtri_work failed");
        }
        return b;
#else
    ...
    auto solve(VecArithmetic auto b) const -> decltype(b) {
        ...
        if (info != 0) {
            throw std::runtime_error("QrDcmp: LAPACKE_strtrs_work failed");
        }
        return b;
#else
