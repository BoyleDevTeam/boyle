...
class VectorX final {
  ...
  public:
    ...
    explicit VectorX(size_type size)
        : m_data(size) {
        m_data.shrink_to_fit();
    }

    [[using gnu: always_inline]]
    explicit VectorX(size_type size, const_reference value)
        : m_data(size, value) {
        m_data.shrink_to_fit();
    }

    [[using gnu: always_inline]]
    VectorX(std::initializer_list<value_type> data)
        : m_data(data) {}

    [[using gnu: always_inline]]
    VectorX(std::vector<value_type, allocator_type> data)
        : m_data{std::move(data)} {
        m_data.shrink_to_fit();
    }

    [[using gnu: always_inline]]
    explicit VectorX(VectorView<value_type> vector_view)
        : m_data(vector_view.size()) {
        m_data.shrink_to_fit();
    ...
    [[using gnu: always_inline]]
    explicit VectorX(VectorView<const value_type> vector_view)
        : m_data(vector_view.size()) {
        m_data.shrink_to_fit();
        for (size_type i{0}; i < vector_view.size(); ++i) {
            operator[](i) = vector_view[i];
    ...
    [[using gnu: always_inline]]
    explicit VectorX(MatrixView<value_type> matrix_view)
    ...
    [[using gnu: always_inline]]
    explicit VectorX(MatrixView<const value_type> matrix_view)
        : m_data(matrix_view.size()) {
#if BOYLE_CHECK_PARAMS == 1
        if (matrix_view.nrows() != 1 && matrix_view.ncols() != 1) [[unlikely]] {
            throw std::out_of_range("this matrix does not covert to a vector.");
    ...
    auto operator/=(const ScalarArithmetic auto& den) noexcept -> VectorX& {
        if constexpr (std::is_floating_point_v<value_type> || isComplexArithmeticV<value_type>) {
