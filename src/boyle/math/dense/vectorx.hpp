...
class VectorX final {
  ...
  public:
    ...
    template <typename U>
    [[using gnu: always_inline]]
    VectorX(std::span<U> data, const allocator_type& alloc = {})
        requires std::is_same_v<std::remove_const_t<U>, value_type>
        : m_data(data.cbegin(), data.cend(), alloc) {
        m_data.shrink_to_fit();
    }
    ...
    [[using gnu: pure, always_inline]] operator std::span<value_type>() noexcept {
        return std::span<value_type>(m_data);
    }

    [[using gnu: pure, always_inline]] operator std::span<const value_type>() const noexcept {
