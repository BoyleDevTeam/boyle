...
class alignas(32) Vector final {
  ...
  public:
    ...
    template <typename U>
    [[using gnu: always_inline]] constexpr Vector(
    ...
    [[using gnu: pure, always_inline]] constexpr operator std::span<value_type, kSize>() noexcept {
        return std::span<value_type, kSize>(m_data);
    }

    [[using gnu: pure, always_inline]] constexpr operator std::span<
        const value_type, kSize>() const noexcept {
