...
template <ScalarArithmetic Scalar, std::size_t Size, typename Alloc>
class alignas(32) Vector final {
  ...
  public:
    ...
    constexpr Vector([[maybe_unused]] const allocator_type& alloc = {}) noexcept {}
    ...
    [[using gnu: always_inline]]
    constexpr explicit Vector(
        VectorView<value_type> vector_view, [[maybe_unused]] const allocator_type& alloc = {}
    ) noexcept(!BOYLE_CHECK_PARAMS) {
#if BOYLE_CHECK_PARAMS == 1
    ...
    [[using gnu: always_inline]]
    constexpr explicit Vector(
        VectorView<const value_type> vector_view, [[maybe_unused]] const allocator_type& alloc = {}
    ) noexcept(!BOYLE_CHECK_PARAMS) {
#if BOYLE_CHECK_PARAMS == 1
        if (vector_view.size() != kSize) [[unlikely]] {
            throw std::invalid_argument("Vector constructor: size mismatches!");
    ...
    [[using gnu: always_inline]]
    constexpr explicit Vector(
        MatrixView<value_type> matrix_view, [[maybe_unused]] const allocator_type& alloc = {}
    ) noexcept(!BOYLE_CHECK_PARAMS)
    ...
    [[using gnu: always_inline]]
    constexpr explicit Vector(
        MatrixView<const value_type> matrix_view, [[maybe_unused]] const allocator_type& alloc = {}
    ) noexcept(!BOYLE_CHECK_PARAMS)
        : m_data(matrix_view.size()) {
#if BOYLE_CHECK_PARAMS == 1
        if (matrix_view.nrows() != 1 && matrix_view.ncols() != 1) [[unlikely]] {
