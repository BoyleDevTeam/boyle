...
class Vector final {
  public:
    ...
    constexpr explicit Vector([[maybe_unused]] size_type size) noexcept(!BOYLE_CHECK_PARAMS) {
    ...
    [[using gnu: always_inline]]
    constexpr explicit Vector([[maybe_unused]] size_type size, const_reference value) noexcept(
        !BOYLE_CHECK_PARAMS
    ) {
#if BOYLE_CHECK_PARAMS == 1
        if (size != kSize) {
    ...
    [[using gnu: always_inline]]
    constexpr Vector(std::initializer_list<value_type> list) noexcept(!BOYLE_CHECK_PARAMS)
        : m_data{list} {
#if BOYLE_CHECK_PARAMS == 1
        if (list.size() != kSize) {
            throw std::invalid_argument("Vector constructor: size mismatches!");
    ...
    [[using gnu: always_inline]]
    constexpr Vector(const std::array<value_type, kSize>& data) noexcept(!BOYLE_CHECK_PARAMS)
        : m_data{data} {
#if BOYLE_CHECK_PARAMS == 1
        if (data.size() != kSize) {
            throw std::invalid_argument("Vector constructor: size mismatches!");
    ...
    [[using gnu: always_inline]]
    constexpr explicit Vector(VectorView<value_type> vector_view) noexcept(!BOYLE_CHECK_PARAMS) {
#if BOYLE_CHECK_PARAMS == 1
        if (vector_view.size() != kSize) {
            throw std::invalid_argument("Vector constructor: size mismatches!");
        }
#endif
    ...
    [[using gnu: always_inline]]
    constexpr explicit Vector(VectorView<const value_type> vector_view) noexcept(!BOYLE_CHECK_PARAMS
    ) {
#if BOYLE_CHECK_PARAMS == 1
        if (vector_view.size() != kSize) {
            throw std::invalid_argument("Vector constructor: size mismatches!");
    ...
    [[using gnu: always_inline]]
    constexpr explicit Vector(MatrixView<value_type> matrix_view) noexcept(!BOYLE_CHECK_PARAMS)
    ...
    [[using gnu: always_inline]]
    constexpr explicit Vector(MatrixView<const value_type> matrix_view) noexcept(!BOYLE_CHECK_PARAMS
    )
        : m_data(matrix_view.size()) {
#if BOYLE_CHECK_PARAMS == 1
        if (matrix_view.nrows() != 1 && matrix_view.ncols() != 1) {
    ...
    constexpr operator VectorView<value_type>() noexcept {
        return VectorView<value_type>(m_data.size(), m_data.data());
    }

    [[using gnu: pure, always_inline]]
    constexpr operator VectorView<const value_type>() const noexcept {
        return VectorView<const value_type>(m_data.size(), m_data.data());
    }
    ...
    [[using gnu: pure, always_inline, hot]]
    constexpr auto identicalTo(VectorView<value_type> obj, double tol = 1E-8) const noexcept
        -> bool {
        if (kSize != obj.size()) {
            return false;
        }
  ...
  private:
    alignas(32) std::array<value_type, Size> m_data;
};
