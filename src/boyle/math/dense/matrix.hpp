...
class Matrix final {
  public:
    ...
    static constexpr std::size_t kNRows = NRows;
    static constexpr std::size_t kNCols = NCols;
    ...
    [[using gnu: always_inline]]
    constexpr explicit Matrix(MatrixView<value_type, kOrder> matrix_view
    ) noexcept(!BOYLE_CHECK_PARAMS) {
#if BOYLE_CHECK_PARAMS == 1
        if (matrix_view.nrows() != kNRows || matrix_view.ncols() != kNCols) {
            throw std::invalid_argument("Matrix constructor: mismatch matrix sizes detected.");
        }
#endif
    ...
    [[using gnu: always_inline]]
    constexpr explicit Matrix(MatrixView<const value_type, kOrder> matrix_view
    ) noexcept(!BOYLE_CHECK_PARAMS) {
#if BOYLE_CHECK_PARAMS == 1
        if (matrix_view.nrows() != kNRows || matrix_view.ncols() != kNCols) {
            throw std::invalid_argument("Matrix constructor: mismatch matrix sizes detected.");
    ...
    constexpr operator MatrixView<value_type, kOrder>() noexcept {
        return MatrixView<value_type, kOrder>(kNRows, kNCols, m_data.data());
    }

    [[using gnu: pure, always_inline]]
    constexpr operator MatrixView<const value_type, kOrder>() const noexcept {
        return MatrixView<const value_type, kOrder>(kNRows, kNCols, m_data.data());
    }
    ...
    [[using gnu: always_inline, hot]]
    constexpr auto operator-(const Matrix& obj) && noexcept(!BOYLE_CHECK_PARAMS) -> Matrix&& {
        operator-=(obj);
        return std::move(*this);
    }
    ...
    [[using gnu: pure, always_inline, hot]]
    constexpr auto identicalTo(MatrixView<value_type, kOrder> obj, double tol = 1E-8) const noexcept
        -> bool {
    ...
    constexpr auto transposed() const noexcept -> Matrix<value_type, kNCols, kNRows, kOrder> {
        Matrix<value_type, kNCols, kNRows> result;
        for (size_type i{0}; i < kNRows; ++i) {
            for (size_type j{0}; j < kNCols; ++j) {
    ...
    constexpr auto dot(const Matrix<value_type, kNCols, OuterSize, kOrder>& obj) const noexcept
        ...
        constexpr blasint lda = (kOrder == ::boyle::math::MatrixOrder::COL_MAJOR ? kNRows : kNCols);
        constexpr blasint ldb =
            (kOrder == ::boyle::math::MatrixOrder::COL_MAJOR ? kNCols : OuterSize);
        constexpr blasint ldc =
        ...
        for (std::size_t i{0}; i < kNRows; ++i) {
            for (std::size_t j{0}; j < OuterSize; ++j) {
                for (std::size_t k{0}; k < kNCols; ++k) {
  ...
  private:
    ...
    alignas(32) std::array<value_type, kNRows * kNCols> m_data;
};
