...
class alignas(32) Matrix final {
  ...
  public:
    ...
    [[using gnu: always_inline]]
    constexpr explicit Matrix(MatrixView<value_type, kOrder> matrix_view) noexcept(
        !BOYLE_CHECK_PARAMS
    ) {
#if BOYLE_CHECK_PARAMS == 1
    ...
    [[using gnu: always_inline]]
    constexpr explicit Matrix(MatrixView<const value_type, kOrder> matrix_view) noexcept(
        !BOYLE_CHECK_PARAMS
    ) {
#if BOYLE_CHECK_PARAMS == 1
        if (matrix_view.nrows() != kNRows || matrix_view.ncols() != kNCols) [[unlikely]] {
