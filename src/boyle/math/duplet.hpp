...
#define DECLARE_DUPLET(ClassName, VAR_0, VAR_1)                                                 \
    ...
    class ClassName final : public ::boyle::math::Duplet<T0, T1> {                              \
      ...
      public:                                                                                   \
        ...
        constexpr                                                                               \
        operator ClassName<U0, U1>() const noexcept {                                           \
            return ClassName<U0, U1>{static_cast<U0>(m_first), static_cast<U1>(m_second)};      \
        }                                                                                       \
                                                                                                \
...
class Duplet {
  ...
  public:
    ...
    [[using gnu: always_inline]] constexpr Duplet(Duplet&& other) noexcept = default;
    [[using gnu: always_inline]]
    constexpr auto
    operator=(const Duplet& other) noexcept -> Duplet& = default;
    [[using gnu: always_inline]]
    constexpr auto
    operator=(Duplet&& other) noexcept -> Duplet& = default;
    virtual ~Duplet() noexcept = 0;

    [[using gnu: always_inline]] constexpr Duplet(const std::pair<T0, T1>& pair) noexcept
        : m_first{pair.first}, m_second{pair.second} {}

    [[using gnu: pure, always_inline]]
    constexpr
    operator std::pair<T0, T1>() const noexcept {
...
[[using gnu: always_inline]]
inline auto
operator<<(std::basic_ostream<CharT>& os, const boyle::math::Duplet<T0, T1>& obj) noexcept
    -> std::basic_ostream<CharT>& {
