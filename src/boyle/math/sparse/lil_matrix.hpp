...
class LilMatrix final {
  ...
  public:
    ...
    explicit LilMatrix(const allocator_type& alloc = {}) noexcept
        : m_row_dictionaries(typename dictionary_type::allocator_type{alloc}) {}
    ...
    explicit LilMatrix(size_type nrows, size_type ncols, const allocator_type& alloc = {}) noexcept
        : m_nrows{nrows}, m_ncols{ncols}, m_row_dictionaries(alloc) {}

    [[using gnu: always_inline]]
    LilMatrix(const DokMatrix<value_type, index_type, allocator_type>& dok_matrix) noexcept
    ...
    [[using gnu: ]] operator DokMatrix<value_type, index_type, allocator_type>() const noexcept {
    ...
    [[using gnu: flatten, leaf, hot]]
    auto updateRow(
        size_type row, const DokVector<value_type, index_type, OtherAlloc>& row_vector
    ) noexcept -> void {
#ifndef BOYLE_USE_BOOST_UNORDERED
        using std::erase_if;
#else
        ...
        typename DokVector<value_type, index_type, allocator_type>::dictionary_type row_dictionary(
            row_vector.dictionary().cbegin(), row_vector.dictionary().cend(), m_ncols,
            get_allocator()
        );
        erase_if(
            row_dictionary,
