...
class LilMatrix final {
  ...
  public:
    ...
    template <Allocatory OtherAlloc = ::boyle::common::AlignedAllocator<value_type, 32>>
    [[using gnu: flatten, leaf, hot]]
    auto updateRow(size_type row, const DokVector<value_type, index_type, OtherAlloc>& row_vector)
        -> void {
#ifndef BOYLE_USE_BOOST_UNORDERED
        using std::erase_if;
#else
