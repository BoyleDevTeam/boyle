...
class CooMatrix final {
  ...
  public:
    ...
    explicit CooMatrix(const allocator_type& alloc = {}) noexcept
        : m_offsets_map(typename dictionary_type::allocator_type{alloc}), m_values(alloc),
          m_row_indices(
    ...
    [[using gnu: ]]
    CooMatrix(const DokMatrix<value_type, index_type, allocator_type>& dok_matrix) noexcept
        : m_nrows{dok_matrix.nrows()}, m_ncols{dok_matrix.ncols()},
          m_offsets_map(typename dictionary_type::allocator_type{dok_matrix.get_allocator()}),
          m_values(dok_matrix.get_allocator()),
          m_row_indices(
              typename std::allocator_traits<allocator_type>::template rebind_alloc<index_type>(
