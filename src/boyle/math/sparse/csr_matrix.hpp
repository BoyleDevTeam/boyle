...
class [[nodiscard]] CsrMatrix final {
  ...
  public:
    ...
    [[using gnu: always_inline]]
    explicit CsrMatrix(size_type nrows, size_type ncols, const allocator_type& alloc = {}) noexcept
        : m_ncols{ncols}, m_values(alloc),
          m_inner_indices(
              typename std::allocator_traits<allocator_type>::template rebind_alloc<index_type>(
                  alloc
              )
    ...
    [[using gnu: ]]
    CsrMatrix(const DokMatrix<value_type, index_type, allocator_type>& dok_matrix) noexcept
        : m_ncols{dok_matrix.ncols()}, m_values(dok_matrix.get_allocator()),
          m_inner_indices(
              typename std::allocator_traits<allocator_type>::template rebind_alloc<index_type>{
                  dok_matrix.get_allocator()
              }
          ),
          m_outer_indices(
              typename std::allocator_traits<allocator_type>::template rebind_alloc<index_type>{
        ...
        auto [_, row_major_dictionary_end] = std::ranges::partial_sort_copy(
            ...
            [](const std::pair<IndexPair<index_type>, value_type>& a,
               const std::pair<IndexPair<index_type>, value_type>& b
            ) static constexpr noexcept -> bool {
