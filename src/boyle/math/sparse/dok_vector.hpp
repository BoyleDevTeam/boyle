...
class DokVector final {
  ...
  public:
    ...
    DokVector(const DokVector& other) = default;
    auto operator=(const DokVector& other) -> DokVector& = default;
    ...
    auto updateCoeff(size_type i, const_reference value) -> void {
        if (i >= m_size) [[unlikely]] {
            return;
        }
    ...
    [[using gnu: flatten, leaf, hot]]
    auto eraseCoeff(size_type i) -> void {
        if (i >= m_size) [[unlikely]] {
            return;
        }
