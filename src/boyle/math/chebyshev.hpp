...
class [[nodiscard]] Chebyshev final {
  ...
  public:
    [[using gnu: flatten, leaf]] explicit Chebyshev(
    ...
    auto
    operator()(Scalar t) const noexcept -> Scalar {
        return operator()(t, m_num_trunc);
    }

    [[using gnu: pure, always_inline, leaf]]
    auto
    operator()(Scalar t, Index num_trunc) const noexcept -> Scalar {
  ...
  private:
    [[using gnu: always_inline, leaf]] explicit Chebyshev(
        std::vector<Scalar> coeffs, Scalar lower_bound, Scalar upper_bound
    ) noexcept
