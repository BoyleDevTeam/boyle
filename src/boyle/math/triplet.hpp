...
#define DECLARE_TRIPLET(ClassName, VAR_0, VAR_1, VAR_2)                                         \
    ...
    class ClassName final : public ::boyle::math::Triplet<T0, T1, T2> {                         \
        ...
        using ::boyle::math::Triplet<T0, T1, T2>::m_third;                                      \
                                                                                                \
      public:                                                                                   \
        [[using gnu: always_inline]] ClassName() noexcept = default;                            \
        ...
        [[using gnu: always_inline]] constexpr ClassName(const ClassName& other                 \
        ) noexcept = default;                                                                   \
        [[using gnu: always_inline]] constexpr ClassName(ClassName&& other) noexcept = default; \
        [[using gnu: always_inline]]                                                            \
        constexpr auto                                                                          \
        operator=(const ClassName& other) noexcept -> ClassName& = default;                     \
        [[using gnu: always_inline]]                                                            \
        constexpr auto                                                                          \
        operator=(ClassName&& other) noexcept -> ClassName& = default;                          \
        ...
        constexpr                                                                               \
        operator ClassName<U0, U1, U2>() const noexcept {                                       \
            return ClassName<T0, T1, T2>{                                                       \
                static_cast<U0>(m_first), static_cast<U1>(m_second), static_cast<U2>(m_third)   \
            };                                                                                  \
        }                                                                                       \
                                                                                                \
...
class Triplet {
  ...
  public:
    ...
    [[using gnu: always_inline]] constexpr Triplet(Triplet&& other) noexcept = default;
    [[using gnu: always_inline]]
    constexpr auto
    operator=(const Triplet& other) noexcept -> Triplet& = default;
    [[using gnu: always_inline]]
    constexpr auto
    operator=(Triplet&& other) noexcept -> Triplet& = default;
    virtual ~Triplet() noexcept = 0;

    [[using gnu: always_inline]] constexpr Triplet(const std::tuple<T0, T1, T2>& tuple) noexcept
        : m_first{std::get<0>(tuple)}, m_second{std::get<1>(tuple)}, m_third{std::get<2>(tuple)} {}

    [[using gnu: pure, always_inline]]
    constexpr
    operator std::tuple<T0, T1, T2>() const noexcept {
...
[[using gnu: always_inline]]
inline auto
operator<<(std::basic_ostream<CharT>& os, const boyle::math::Triplet<T0, T1, T2>& obj) noexcept
