...
template <
    ComplexArithmetic Complex, typename Alloc = ::boyle::common::AlignedAllocator<Complex, 32>>
[[using gnu: pure]] [[nodiscard]]
inline auto fft(
    std::span<const Complex> input, FftNorm norm = FftNorm::BACKWARD, const Alloc& alloc = Alloc()
) noexcept(!BOYLE_CHECK_PARAMS) -> std::vector<Complex, Alloc> {
    ...
    std::vector<Complex, Alloc> output(num_pnts, alloc);
...
template <
    ...
    typename Alloc = ::boyle::common::AlignedAllocator<std::complex<Scalar>, 32>>
[[using gnu: pure]] [[nodiscard]]
inline auto rfft(
    std::span<const Scalar> input, FftNorm norm = FftNorm::BACKWARD, const Alloc& alloc = Alloc()
) noexcept(!BOYLE_CHECK_PARAMS) -> std::vector<std::complex<Scalar>, Alloc> {
    const std::size_t num_pnts{input.size()};
#if BOYLE_CHECK_PARAMS == 1
    if (num_pnts < 2) [[unlikely]] {
    ...
    std::vector<Complex, typename std::allocator_traits<Alloc>::template rebind_alloc<Complex>>
        output(num_pnts / 2 + 1, alloc);
...
template <
    ComplexArithmetic Complex,
    typename Alloc = ::boyle::common::AlignedAllocator<typename Complex::value_type, 32>>
[[using gnu: pure]] [[nodiscard]]
inline auto irfft(
    std::span<const Complex> input, FftNorm norm = FftNorm::BACKWARD, const Alloc& alloc = Alloc()
) noexcept(!BOYLE_CHECK_PARAMS) -> std::vector<typename Complex::value_type, Alloc> {
    const std::size_t num_pnts{input.size()};
#if BOYLE_CHECK_PARAMS == 1
    if (num_pnts < 2) [[unlikely]] {
    ...
    std::vector<Scalar, Alloc> output(num_pnts * 2 - 2, alloc);
...
inline auto fftfreq(std::size_t n, Scalar spacing = 1.0, const Alloc& alloc = Alloc()) noexcept
    -> std::vector<Scalar, Alloc> {
    const Scalar val = 1.0 / (n * spacing);
    const int mid = (n - 1) / 2 + 1;
    const int n2 = n / 2;

    std::vector<Scalar, Alloc> output(n, alloc);
    for (int i = 0; i < mid; ++i) {
        output[i] = i * val;
    }
...
template <
    std::floating_point Scalar, typename Alloc = ::boyle::common::AlignedAllocator<Scalar, 32>>
[[using gnu: const, always_inline]] [[nodiscard]]
inline auto rfftfreq(std::size_t n, Scalar spacing = 1.0, const Alloc& alloc = Alloc()) noexcept
    -> std::vector<Scalar, Alloc> {
    const Scalar val = 1.0 / (n * spacing);
    const std::size_t mid = n / 2 + 1;

    std::vector<Scalar, Alloc> output(mid, alloc);
