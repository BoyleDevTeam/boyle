...
class [[nodiscard]] PiecewiseLinearFunction1 final {
  ...
  public:
    ...
    [[using gnu: flatten, leaf]] explicit PiecewiseLinearFunction1(
    ...
    auto derivative(U t, [[maybe_unused]] unsigned int order) const noexcept(!BOYLE_CHECK_PARAMS)
        -> T {
#if BOYLE_CHECK_PARAMS == 1
        if (order != 1) {
            throw std::invalid_argument(std::format(
