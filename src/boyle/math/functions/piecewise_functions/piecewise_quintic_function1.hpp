...
class PiecewiseQuinticFunction1 final {
  ...
  public:
    ...
    static constexpr U kDuplicateCriterion{1E-8};
    ...
    auto operator=(const PiecewiseQuinticFunction1& other
    ) noexcept -> PiecewiseQuinticFunction1& = default;
    PiecewiseQuinticFunction1(PiecewiseQuinticFunction1&& other) noexcept = default;
    auto operator=(PiecewiseQuinticFunction1&& other
    ) noexcept -> PiecewiseQuinticFunction1& = default;
    ~PiecewiseQuinticFunction1() noexcept = default;
  ...
  private:
    ...
    explicit PiecewiseQuinticFunction1(
        ...
        : m_ts{std::move(ts)}, m_ys{std::move(ys)}, m_ddys{std::move(ddys)},
        ...
        } else if (m_ts.size() != m_ys.size()) [[unlikely]] {
            throw std::invalid_argument(fmt::format(
                "Invalid arguments detected! ts, ys must share the same size: ts.size() = {0:d} "
                "while ys.size() = {1:d}",
                ts.size(), ys.size()
            ));
        } else if (!std::is_sorted(m_ts.cbegin(), m_ts.cend())) [[unlikely]] {
            throw std::invalid_argument(
                fmt::format("Invalid arguments detected! ts has to be a sorted array!")
            );
        } else if (hasDuplicates(
                       std::ranges::subrange{m_ts.cbegin(), m_ts.cend()}, kDuplicateCriterion
                   )) [[unlikely]] {
            throw std::invalid_argument(
                fmt::format("Invalid arguments detected! ts can not have duplicated elements!")
            );
