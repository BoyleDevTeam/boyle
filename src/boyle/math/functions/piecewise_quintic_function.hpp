...
class PiecewiseQuinticFunction final {
  ...
  public:
    ...
    PiecewiseQuinticFunction(const PiecewiseQuinticFunction& other) noexcept = default;
    auto operator=(const PiecewiseQuinticFunction& other) noexcept
    ...
    explicit PiecewiseQuinticFunction(const allocator_type& alloc) noexcept
        : m_ts(alloc), m_ys(alloc), m_ddys(alloc), m_d4ys(alloc) {}
    ...
    explicit PiecewiseQuinticFunction(
        std::span<const param_type> ts, std::span<const value_type> ys,
        std::array<BoundaryMode<value_type>, 2> b0, std::array<BoundaryMode<value_type>, 2> bf,
        const allocator_type& alloc = {}
    ) noexcept(!BOYLE_CHECK_PARAMS)
        : m_ts(ts.cbegin(), ts.end(), alloc), m_ys(ys.cbegin(), ys.end(), alloc),
