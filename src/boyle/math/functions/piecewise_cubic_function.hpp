...
template <GeneralArithmetic T, typename Alloc = ::boyle::common::AlignedAllocator<T, 32>>
class PiecewiseCubicFunction final {
  ...
  public:
    ...
    PiecewiseCubicFunction(const PiecewiseCubicFunction& other) noexcept = default;
    auto operator=(const PiecewiseCubicFunction& other) noexcept
        -> PiecewiseCubicFunction& = default;
    ...
    explicit PiecewiseCubicFunction(const allocator_type& alloc) noexcept
        : m_ts(alloc), m_ys(alloc), m_ddys(alloc) {}

    [[using gnu: always_inline]]
    explicit PiecewiseCubicFunction(
        std::span<const param_type> ts, std::span<const value_type> ys,
    ...
    [[using gnu: ]]
    explicit PiecewiseCubicFunction(
        std::span<const param_type> ts, std::span<const value_type> ys, BoundaryMode<value_type> b0,
        BoundaryMode<value_type> bf, const allocator_type& alloc = {}
        ...
        if (hasDuplicates(m_ts.cbegin(), m_ts.cend(), kDuplicateCriterion)) [[unlikely]] {
            throw std::invalid_argument(
    ...
    [[using gnu: ]]
    explicit PiecewiseCubicFunction(
        [[maybe_unused]] periodic_tag tag, std::span<const param_type> ts,
        std::span<const value_type> ys, const allocator_type& alloc = {}
    ) noexcept(!BOYLE_CHECK_PARAMS)
    ...
    auto eval(param_type t) const noexcept -> value_type {
        ...
        const size_type pos = nearestUpperElement(m_ts.cbegin(), m_ts.cend(), t) - m_ts.cbegin();
        if (pos == 0) {
    ...
    {
        size_type pos = std::min_element(m_ys.cbegin(), m_ys.cend()) - m_ys.cbegin();
        param_type h, ratio;
    ...
    auto maxY() const noexcept -> value_type
        requires std::floating_point<value_type>
    {
        size_type pos = std::max_element(m_ys.cbegin(), m_ys.cend()) - m_ys.cbegin();
        param_type h, ratio;
        value_type derivative, derivative2;
        if (pos == 0) {
            h = m_ts[1] - m_ts[0];
...
template <typename Alloc = ::boyle::common::AlignedAllocator<float, 32>>
using PiecewiseCubicFunction1s = PiecewiseCubicFunction<float, Alloc>;

template <typename Alloc = ::boyle::common::AlignedAllocator<double, 32>>
