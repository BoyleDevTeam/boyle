...
class [[nodiscard]] RouteLine2 final {
  ...
  public:
    ...
    auto
    operator()(T s) const noexcept -> ::boyle::math::Vec2<T> {
        return m_curve(s);
    }

    [[using gnu: pure, always_inline]]
    auto
    operator()(T s, T l) const noexcept -> ::boyle::math::Vec2<T> {
        return m_curve(s, l);
    }

    [[using gnu: pure, always_inline]]
    auto
    operator()(::boyle::math::SlDuplet<T> sl) const noexcept -> ::boyle::math::Vec2<T> {
        return m_curve(sl);
    }
    ...
    auto
    operator()(ForwardIt first, std::sentinel_for<ForwardIt> auto last) const noexcept
  ...
  private:
    [[using gnu: always_inline]] explicit RouteLine2(::boyle::math::PiecewiseQuinticCurve2<T> curve
    ) noexcept
