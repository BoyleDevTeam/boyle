...
class RouteLineCubicOffsetModel final {
  public:
    using value_type = T;
    ...
    RouteLineCubicOffsetModel(const RouteLineCubicOffsetModel& other) noexcept = delete;
    auto operator=(const RouteLineCubicOffsetModel& other) noexcept
        -> RouteLineCubicOffsetModel& = delete;
    RouteLineCubicOffsetModel(RouteLineCubicOffsetModel&& other) noexcept = delete;
    auto operator=(RouteLineCubicOffsetModel&& other) noexcept
        -> RouteLineCubicOffsetModel& = delete;
    ...
    explicit RouteLineCubicOffsetModel(
        std::span<const value_type> sketch_points, std::span<const scalar_type> sample_ss,
        ...
        : m_num_samples(sample_ss.size()), m_length_scale(sample_ss.back() - sample_ss.front()),
          m_qp_problem(sample_ss.size() * 4, sample_ss.size() * 6, settings.memory_resource),
          m_settings{settings},
    ...
    auto setHardBorders(std::span<const HardBorder<value_type>> hard_borders) noexcept -> void {
        ...
        for (const auto& hard_border : hard_borders) {
            ...
            if (hard_border.chirality == ::boyle::kinetics::Chirality::RIGHT) {
                if (istart != 0) {
                    ...
                    const std::array<scalar_type, 4> proration_coeffs =
                        ::boyle::math::cuberpCoeffs(ratio, h);
                    boyle::math::pmr::DokVector<scalar_type, index_type> constraint_vec{
                    ...
                    m_qp_problem.addConstrainTerm(
                        constraint_vec, bound_points.front().x,
                        std::numeric_limits<scalar_type>::max()
                    );
                    constraint_vec = boyle::math::pmr::DokVector<scalar_type, index_type>{
                    ...
                    m_qp_problem.addConstrainTerm(
                        constraint_vec, bound_points.front().y,
                        std::numeric_limits<scalar_type>::max()
                    );
                }
                ...
                if (iend != m_num_samples) {
                    ...
                    const std::array<scalar_type, 4> proration_coeffs =
                        ::boyle::math::cuberpCoeffs(ratio, h);
                    boyle::math::pmr::DokVector<scalar_type, index_type> constrain_vec{
                    ...
                    m_qp_problem.addConstrainTerm(
                        constrain_vec, bound_points.back().x,
                        std::numeric_limits<scalar_type>::max()
                    );
                    constrain_vec = boyle::math::pmr::DokVector<scalar_type, index_type>{
                    ...
                    m_qp_problem.addConstrainTerm(
                        constrain_vec, bound_points.back().y,
                        std::numeric_limits<scalar_type>::max()
                    );
                }
            } else {
                if (istart != 0) {
                    ...
                    m_qp_problem.addConstrainTerm(
                        constrain_vec, std::numeric_limits<scalar_type>::lowest(),
                        bound_points.front().x
                    );
                    constrain_vec = boyle::math::pmr::DokVector<scalar_type, index_type>{
                        {{yIndex(istart - 1), proration_coeffs[0]},
                         {yIndex(istart), proration_coeffs[1]},
        ...
        for (index_type i{1}; i < m_num_samples - 1; ++i) {
            m_qp_problem.updateConstrainTerm(
                xIndex(i),
                boyle::math::pmr::DokVector<scalar_type, index_type>{
                    {{xIndex(i), 1.0}}, get_allocator()
                },
                x_lower_bound[i], x_upper_bound[i]
            );
            m_qp_problem.updateConstrainTerm(
                yIndex(i),
                boyle::math::pmr::DokVector<scalar_type, index_type>{
                    {{yIndex(i), 1.0}}, get_allocator()
                },
                y_lower_bound[i], y_upper_bound[i]
            );
        }
        return;
    }
    ...
    auto setSoftBorders(std::span<const SoftBorder<value_type>> soft_borders) noexcept -> void {
        for (const auto& soft_border : soft_borders) {
            const index_type istart = ::boyle::math::nearestUpperElement(
                                          m_sample_points.cbegin(), m_sample_points.cend(),
            ...
            if (soft_border.chirality == ::boyle::kinetics::Chirality::RIGHT) {
                if (istart == iend) {
                    ...
                    std::array<scalar_type, 4> proration_coeffs =
                        ::boyle::math::cuberpCoeffs(ratio, h);
                    boyle::math::pmr::DokVector<scalar_type, index_type> constrain_vec{
                        {{xIndex(istart - 1), -proration_coeffs[0]},
                         {xIndex(istart), -proration_coeffs[1]},
                    ...
                    m_qp_problem.addRampCostTerm(
                        constrain_vec, -bound_points.front().x, soft_border.linear_weight * factor,
                    ...
                    m_qp_problem.addRampCostTerm(
                        constrain_vec, -bound_points.front().y, soft_border.linear_weight * factor,
                        soft_border.quadratic_weight * factor
                    );
                    ...
                    m_qp_problem.addRampCostTerm(
                        constrain_vec, -bound_points.back().x, soft_border.linear_weight * factor,
                        soft_border.quadratic_weight * factor
                    );
                    constrain_vec = boyle::math::pmr::DokVector<scalar_type, index_type>{
                        {{yIndex(iend - 1), -proration_coeffs[0]},
                    ...
                    m_qp_problem.addRampCostTerm(
                        constrain_vec, -bound_points.back().y, soft_border.linear_weight * factor,
                        soft_border.quadratic_weight * factor
                    );
                    break;
                }
                if (istart != 0) {
                    ...
                    m_qp_problem.addRampCostTerm(
                        constraint_vec, -bound_points.front().x, soft_border.linear_weight * factor,
                        soft_border.quadratic_weight * factor
                    );
                    constraint_vec = boyle::math::pmr::DokVector<scalar_type, index_type>{
                        {{yIndex(istart - 1), -proration_coeffs[0]},
                    ...
                    m_qp_problem.addRampCostTerm(
                        constraint_vec, -bound_points.front().y, soft_border.linear_weight * factor,
                        soft_border.quadratic_weight * factor
                    );
                }
                for (index_type i{istart}; i < iend; ++i) {
                    ...
                    m_qp_problem.addRampCostTerm(
                        ...
                        -bound_point.x, soft_border.linear_weight * factor,
                        soft_border.quadratic_weight * factor
                    );
                    m_qp_problem.addRampCostTerm(
                        boyle::math::pmr::DokVector<scalar_type, index_type>{
                        ...
                        -bound_point.y, soft_border.linear_weight * factor,
                        soft_border.quadratic_weight * factor
                    );
                }
                if (iend != m_num_samples) {
    ...
    auto setInitialState(
        ...
        m_qp_problem.updateConstrainTerm(
            xIndex(0),
            ::boyle::math::pmr::DokVector<scalar_type, index_type>{
                {{xIndex(0), 1.0}}, get_allocator()
            },
            r0.x - ::boyle::math::kEpsilon, r0.x + ::boyle::math::kEpsilon
        );
        if (!std::isnan(t0.x)) {
            m_qp_problem.updateConstrainTerm(
                m_num_samples * 4,
        ...
        if (!std::isnan(n0.x)) {
            m_qp_problem.updateConstrainTerm(
                ddxIndex(0),
                ::boyle::math::pmr::DokVector<scalar_type, index_type>{
                    {{ddxIndex(0), 1.0}}, get_allocator()
                },
                n0.x - ::boyle::math::kEpsilon, n0.x + ::boyle::math::kEpsilon
            );
        }
        ...
        if (!std::isnan(n0.y)) {
            m_qp_problem.updateConstrainTerm(
                ddyIndex(0),
                ::boyle::math::pmr::DokVector<scalar_type, index_type>{
                    {{ddyIndex(0), 1.0}}, get_allocator()
                },
                n0.y - ::boyle::math::kEpsilon, n0.y + ::boyle::math::kEpsilon
            );
        }
        return;
    }
    ...
    auto setFinalState(
        ...
        m_qp_problem.updateConstrainTerm(
            xIndex(m_num_samples - 1),
            ::boyle::math::pmr::DokVector<scalar_type, index_type>{
                {{xIndex(m_num_samples - 1), 1.0}}, get_allocator()
            },
        ...
        if (!std::isnan(nf.x)) {
            m_qp_problem.updateConstrainTerm(
                ddxIndex(m_num_samples - 1),
                ::boyle::math::pmr::DokVector<scalar_type, index_type>{
                    {{ddxIndex(m_num_samples - 1), 1.0}}, get_allocator()
                },
        ...
        m_qp_problem.updateConstrainTerm(
            yIndex(m_num_samples - 1),
            ::boyle::math::pmr::DokVector<scalar_type, index_type>{
                {{yIndex(m_num_samples - 1), 1.0}}, get_allocator()
            },
        ...
        if (!std::isnan(nf.y)) {
            m_qp_problem.updateConstrainTerm(
                ddyIndex(m_num_samples - 1),
                ::boyle::math::pmr::DokVector<scalar_type, index_type>{
                    {{ddyIndex(m_num_samples - 1), 1.0}}, get_allocator()
                },
    ...
    auto setOffsetCost(scalar_type offset_weight) noexcept -> void {
        ...
        for (index_type i{0}; i < m_num_samples - 1; ++i) {
            ...
            m_qp_problem.addQuadCostTerm(
                ddxIndex(i), ddxIndex(i + 1), factor * m_h4s[i] * kFactors[4]
            );
            m_qp_problem.addQuadCostTerm(
            ...
            m_qp_problem.addQuadCostTerm(
                ddyIndex(i), ddyIndex(i + 1), factor * m_h4s[i] * kFactors[4]
            );
            m_qp_problem.addQuadCostTerm(
    ...
    auto solve() const
        ...
        constexpr std::array<scalar_type, 2> kFactor{1.0 / 3.0, 1.0 / 6.0};
        ...
        const scalar_type dx0{
            (osqp_result.prim_vars[xIndex(1)] - osqp_result.prim_vars[xIndex(0)]) *
                m_reciprocal_hs[0] -
            (osqp_result.prim_vars[ddxIndex(0)] * kFactor[0] +
             osqp_result.prim_vars[ddxIndex(1)] * kFactor[1]) *
                m_hs[0]
        };
        const scalar_type dxf{
        ...
        const scalar_type dy0{
            (osqp_result.prim_vars[yIndex(1)] - osqp_result.prim_vars[yIndex(0)]) *
                m_reciprocal_hs[0] -
            (osqp_result.prim_vars[ddyIndex(0)] * kFactor[0] +
             osqp_result.prim_vars[ddyIndex(1)] * kFactor[1]) *
                m_hs[0]
        };
        const scalar_type dyf{
  ...
  private:
    auto setIntegrationRelation() noexcept -> void {
        for (index_type i{1}; i < m_num_samples - 1; ++i) {
            ...
            m_qp_problem.updateConstrainTerm(
                m_num_samples * 4 + i, constrain_vec, -::boyle::math::kEpsilon,
                ::boyle::math::kEpsilon
            );
            constrain_vec = boyle::math::pmr::DokVector<scalar_type, index_type>{
            ...
            m_qp_problem.updateConstrainTerm(
                m_num_samples * 5 + i, constrain_vec, -::boyle::math::kEpsilon,
                ::boyle::math::kEpsilon
            );
        }
        return;
