...
RouteLineCubicOffsetModel::RouteLineCubicOffsetModel(
    ...
    const ::boyle::math::OsqpSettings& settings
) noexcept(!BOYLE_CHECK_PARAMS)
    : RouteLineCubicOffsetModel{raw_sketch_points, std::move(sample_ss)} {
    m_settings = settings;
}
...
auto RouteLineCubicOffsetModel::qp_problem() const noexcept
    -> const ::boyle::math::QpProblem<double, int>& {
    return m_qp_problem;
}

auto RouteLineCubicOffsetModel::settings() const noexcept -> const ::boyle::math::OsqpSettings& {
    return m_settings;
}
...
auto RouteLineCubicOffsetModel::setHardBorders(const std::vector<HardBorder2d>& hard_borders
) noexcept -> void {
...
auto RouteLineCubicOffsetModel::setSoftBorders(const std::vector<SoftBorder2d>& soft_borders
) noexcept -> void {
...
auto RouteLineCubicOffsetModel::solve() const -> RouteLineCubicOffsetModel::Result {
    constexpr std::array<double, 2> kFactor{1.0 / 3.0, 1.0 / 6.0};
    const ::boyle::math::OsqpSolver osqp_solver{m_settings};
    const ::boyle::math::OsqpResult osqp_result = osqp_solver.solve(m_qp_problem);
    const double dx0{
    ...
    return RouteLineCubicOffsetModel::Result{
        Path2d{std::move(anchor_points), b0, bf}, osqp_result.info
    };
}
