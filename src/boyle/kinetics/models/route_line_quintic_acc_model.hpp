...
class RouteLineQuinticAccModel final {
  public:
    ...
    explicit RouteLineQuinticAccModel(
        std::span<const scalar_type> sample_ts,
        ...
        for (const auto& hard_fence : hard_fences) {
            ...
            if (hard_fence.actio == ::boyle::kinetics::Actio::PUSHING) {
                if (istart != 0) {
                    ...
                    const boyle::math::pmr::DokVector<scalar_type, index_type> constrain_vec{
                    ...
                    m_qp_problem.addConstrainTerm(
                        constrain_vec, hard_fence.bound_ss.front(),
                ...
                if (iend != m_num_samples) {
                    ...
                    m_qp_problem.addConstrainTerm(
                        constrain_vec, hard_fence.bound_ss.back(),
                        std::numeric_limits<scalar_type>::max()
                    );
                }
            } else {
            ...
            if (soft_fence.actio == ::boyle::kinetics::Actio::PUSHING) {
                if (istart == iend) {
                    ...
                    m_qp_problem.addRampCostTerm(
                        constrain_vec, -soft_fence.bound_ss.front(),
                    ...
                    m_qp_problem.addRampCostTerm(
                        constrain_vec, -soft_fence.bound_ss.back(),
                        soft_fence.linear_weight * factor, soft_fence.quadratic_weight * factor
                    );
                    break;
                }
                ...
                for (index_type i{istart}; i < iend; ++i) {
                    ...
                    m_qp_problem.addRampCostTerm(
                        ...
                        -bound_func(m_sample_ts[i]), soft_fence.linear_weight * factor,
    ...
    auto setInitialState(
        ...
        if (!std::isnan(s0)) {
            m_qp_problem.updateConstrainTerm(
                sIndex(0),
                ::boyle::math::pmr::DokVector<scalar_type, index_type>{
                    {{sIndex(0), 1.0}}, get_allocator()
                },
                s0 - ::boyle::math::kEpsilon, s0 + ::boyle::math::kEpsilon
            );
        }
        if (!std::isnan(v0)) {
            m_qp_problem.updateConstrainTerm(
                vIndex(0),
                ::boyle::math::pmr::DokVector<scalar_type, index_type>{
                    {{vIndex(0), 1.0}}, get_allocator()
                },
                v0 - ::boyle::math::kEpsilon, v0 + ::boyle::math::kEpsilon
            );
        }
        if (!std::isnan(a0)) {
            m_qp_problem.updateConstrainTerm(
    ...
    auto setFinalState(
        ...
        if (!std::isnan(sf)) {
            m_qp_problem.updateConstrainTerm(
                sIndex(m_num_samples - 1),
                ::boyle::math::pmr::DokVector<scalar_type, index_type>{
                    {{sIndex(m_num_samples - 1), 1.0}}, get_allocator()
                },
        ...
        if (!std::isnan(vf)) {
            m_qp_problem.updateConstrainTerm(
                vIndex(m_num_samples - 1),
                ::boyle::math::pmr::DokVector<scalar_type, index_type>{
                    {{vIndex(m_num_samples - 1), 1.0}}, get_allocator()
                },
        ...
        if (!std::isnan(af)) {
            m_qp_problem.updateConstrainTerm(
                aIndex(m_num_samples - 1),
                ::boyle::math::pmr::DokVector<scalar_type, index_type>{
                    {{aIndex(m_num_samples - 1), 1.0}}, get_allocator()
                },
  ...
  private:
    ...
    auto setIntegrationRelation() noexcept -> void {
        for (index_type i{1}; i < m_num_samples - 1; ++i) {
            ...
            m_qp_problem.updateConstrainTerm(
                m_num_samples * 3 + i, constrain_vec, -::boyle::math::kEpsilon,
                ::boyle::math::kEpsilon
            );
            constrain_vec = boyle::math::pmr::DokVector<scalar_type, index_type>{
