...
RouteLineQuinticOffsetModel::RouteLineQuinticOffsetModel(
    ...
    : RouteLineQuinticOffsetModel{std::move(raw_sketch_points), std::move(sample_ss)} {
    m_settings = settings;
}
...
auto RouteLineQuinticOffsetModel::setHardBorders(
    std::span<const HardBorder2d> hard_borders
) noexcept -> void {
...
auto RouteLineQuinticOffsetModel::setSoftBorders(
    std::span<const SoftBorder2d> soft_borders
) noexcept -> void {
...
auto RouteLineQuinticOffsetModel::solve() const
    ...
    ::boyle::math::squeeze(
        ...
        std::ranges::subrange{
            ...
            osqp_result.prim_vars.data() + yIndex(m_num_samples - 1) + 1
        },
        anchor_points.begin()
    );
