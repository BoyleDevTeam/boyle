...
auto RouteLineQuinticAccModel::settings() const noexcept -> const ::boyle::math::OsqpSettings& {
    return m_settings;
}
...
auto RouteLineQuinticAccModel::setHardFences(const std::vector<HardFence1d>& hard_fences) noexcept
    -> void {
...
auto RouteLineQuinticAccModel::setSoftFences(const std::vector<SoftFence1d>& soft_fences) noexcept
    -> void {
...
auto RouteLineQuinticAccModel::solve() const -> RouteLineQuinticAccModel::Result {
    ...
    const std::vector<double> anchor_ss{
        osqp_result.prim_vars.cbegin(),
        osqp_result.prim_vars.cbegin() + sIndex(m_num_samples - 1) + 1
    };
    return Result{Motion1d{m_sample_ts, anchor_ss, b0, bf}, osqp_result.info};
}
