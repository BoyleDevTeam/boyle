...
RouteLineQuinticAccModel::RouteLineQuinticAccModel(std::vector<double> sample_ts) noexcept(
    !BOYLE_CHECK_PARAMS
) {
...
auto RouteLineQuinticAccModel::setHardFences(std::span<const HardFence1d> hard_fences) noexcept
    -> void {
    ...
    for (const HardFence1d& hard_fence : hard_fences) {
        const int istart = ::boyle::math::nearestUpperElement(
                               std::ranges::subrange{m_sample_ts.cbegin(), m_sample_ts.cend()},
                               hard_fence.bound_ts.front()
                           ) -
                           m_sample_ts.cbegin();
        const int iend = ::boyle::math::nearestUpperElement(
                             std::ranges::subrange{m_sample_ts.cbegin(), m_sample_ts.cend()},
                             hard_fence.bound_ts.back()
                         ) -
                         m_sample_ts.cbegin();
        ...
        if (hard_fence.actio == ::boyle::kinetics::Actio::PUSHING) {
            if (istart != 0) {
                ...
                const boyle::math::DokVector<double, int> constrain_vec{
...
auto RouteLineQuinticAccModel::setSoftFences(std::span<const SoftFence1d> soft_fences) noexcept
    -> void {
    for (const SoftFence1d& soft_fence : soft_fences) {
        const int istart = ::boyle::math::nearestUpperElement(
                               std::ranges::subrange{m_sample_ts.cbegin(), m_sample_ts.cend()},
                               soft_fence.bound_ts.front()
                           ) -
                           m_sample_ts.cbegin();
        const int iend = ::boyle::math::nearestUpperElement(
                             std::ranges::subrange{m_sample_ts.cbegin(), m_sample_ts.cend()},
                             soft_fence.bound_ts.back()
                         ) -
                         m_sample_ts.cbegin();
        if (istart == m_num_samples) {
            BOYLE_LOG_WARN(
...
auto RouteLineQuinticAccModel::solve() const
    -> std::pair<Motion1d, ::boyle::cvxopm::Info<double, int>> {
    ...
    std::vector<double> anchor_ss{
        osqp_result.prim_vars.data(), osqp_result.prim_vars.data() + sIndex(m_num_samples - 1) + 1
    };
