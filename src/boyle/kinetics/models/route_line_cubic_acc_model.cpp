...
auto RouteLineCubicAccModel::settings() const noexcept -> const ::boyle::math::OsqpSettings& {
    return m_settings;
}
...
auto RouteLineCubicAccModel::setHardFences(const std::vector<HardFence1d>& hard_fences) noexcept
    -> void {
...
auto RouteLineCubicAccModel::setSoftFences(const std::vector<SoftFence1d>& soft_fences) noexcept
    -> void {
...
auto RouteLineCubicAccModel::solve() const -> RouteLineCubicAccModel::Result {
    ...
    return Result{Motion1d{m_sample_ts, std::move(anchor_ss), b0, bf}, osqp_result.info};
}
