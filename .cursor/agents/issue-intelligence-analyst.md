...
<example>
Context: User is running ideating with a focus on bugs and issue patterns.
user: "/ideating bugs"
...
<commentary>The ideating skill detected issue-tracker intent and dispatches this agent as a third parallel Phase 1 scan alongside codebase context and learnings search.</commentary>
</example>
<example>
...
- `ideating` — as a third parallel Phase 1 scan when issue-tracker intent is detected
...
This report may be consumed by other agents (ideation, planning). Keep output focused:

- Sparse results (<10 issues, 1-2 themes): ~500 tokens
- Typical results (20-50 issues, 3-5 themes): ~1000 tokens
- Rich results: cap at ~1500 tokens

Compress by tightening theme descriptions, not by dropping themes or representative issues.
