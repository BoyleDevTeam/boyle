...
Reliability findings max out at **P1**. Missing timeouts, retry storms, and cascading failures are serious production risks but require runtime conditions to manifest -- they are not immediate defects.

- **P1**: Missing timeout on a critical-path external call; retry loop without backoff that can trigger a storm
- **P2**: Error swallowing that masks failures; missing circuit breaker on a non-critical dependency
- **P3**: Suboptimal error context; catch block that could propagate more information

## Autofix constraints

Default `autofix_class: advisory` with `owner: human`. Adding error handling, timeouts, or retry logic requires understanding the specific failure modes and the acceptable degradation strategy for each dependency.
