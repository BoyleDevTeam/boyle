...
- **C++ memory and lifetime bugs** -- dangling references from temporaries, use-after-free in callback chains, iterator invalidation during container modification, missing virtual destructors in polymorphic hierarchies, uninitialized variables in branches that skip initialization.
- **Coordinate and unit mismatches** -- mixing radians and degrees, confusing ENU/UTM/VRF coordinate systems, applying transforms in wrong order (rotation before translation), timestamp unit mismatches (seconds vs milliseconds vs microseconds).
...
Correctness findings can reach **P0** when a confirmed logic bug has a traceable input-to-wrong-output path. Most findings are P1 (clear bug, moderate blast radius) or P2 (potential bug contingent on runtime conditions).

## Autofix constraints

Default `autofix_class: manual` with `owner: human`. Logic fixes require understanding the intended behavior, not just the code structure. Use `advisory` only for edge-case null checks where the fix is unambiguous.
