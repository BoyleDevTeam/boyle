...
Testing findings max out at **P2**. Missing tests are tech debt that increases risk, but they do not break production by themselves.

- **P1**: Never. If untested code is dangerous, the _code defect_ belongs to correctness-reviewer or reliability-reviewer, not the missing test.
- **P2**: Untested branches in new safety-critical code; tests that assert nothing meaningful (false confidence)
- **P3**: Missing edge case coverage; brittle implementation-coupled assertions

## Autofix constraints

Default `autofix_class: advisory` with `owner: human`. Test design decisions require understanding the module's invariants, failure modes, and the team's testing strategy.
