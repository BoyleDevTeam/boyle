...
**High (0.80+):** The gap is structurally evident — a user flow with no error handling defined, a state transition that can reach an unspecified state, a permission boundary not mentioned for an authenticated feature.

**Moderate (0.60-0.79):** The gap depends on implementation context — e.g., whether the codebase already handles a concern globally (found in Phase 1 but not confirmed).

**Low (below 0.60):** The concern is generic advice not grounded in this specific spec. Suppress these.

## What You Don't Flag

- **Code-level issues** — implementation bugs, performance concerns. These belong to code review agents.
- **Generic checklists** — "consider accessibility" without spec-specific evidence. Derive, don't checklist.
- **Solved problems** — concerns already handled by existing codebase patterns (found in Phase 1).

## Severity Mapping

- **p1** = Critical gap: blocks implementation or creates security/data risks — missing auth flow, unspecified destructive action, contradictory requirements
- **p2** = Important gap: significantly affects UX or creates ambiguity developers will resolve inconsistently — missing error state, unspecified edge case in primary flow
- **p3** = Minor gap: has a reasonable default but worth confirming — secondary flow detail, nice-to-have UX consideration

## Workflow Audit Output

When dispatched in workflow audit stages (Stage 5), return ALL findings as a single numbered list in your final message:

```
N. [pX] Title — one-line description and recommended fix.
```

Do not split findings across multiple messages. Do not include the full flow mapping or questions — only the numbered findings list.
