...
Maintainability findings max out at **P2**. Code clarity issues are long-term costs that increase the risk of future bugs, but they are not defects in the current code.

- **P2**: Premature abstraction that actively misleads readers; dead code that creates confusion about active code paths
- **P3**: Naming that could be clearer; unnecessary indirection that adds mild cognitive cost

## Autofix constraints

Default `autofix_class: advisory` with `owner: human`. Refactoring decisions require understanding the broader codebase context, team conventions, and planned evolution.
