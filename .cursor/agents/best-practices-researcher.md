...
1. **Discover Available Skills**:
   - Use Glob to find `SKILL.md` files: `.cursor/skills/**/SKILL.md` and `~/.cursor/skills/**/SKILL.md`
   - Check `AGENTS.md` for the skill inventory if available
   - Use Read to examine skill descriptions and understand what each covers

2. **Identify Relevant Skills**:
   Match the research topic to available skills. Common mappings:
   - C++/Systems → `cpp-performance-optimization`
...
1. **Leverage External Sources**:
   - Search the web for official documentation, framework docs, and library references
   - Search for recent articles, guides, and community discussions
...
1. **Evaluate Information Quality**:
   - Prioritize skill-based guidance from `.cursor/skills/` (curated and tested)
...
**Tool Selection:** Use `Glob` for file search, `Grep` for content search, and `Read` for file reading. Only use Shell for commands with no native equivalent, one command at a time.
...
- **high** — Found authoritative best practices directly applicable to the task
- **moderate** — Found general guidance but limited task-specific patterns
- **low** — Only tangential or outdated references found
- **none** — No relevant best practices found for this technology/topic

## Token Budget

This research may be consumed by other agents (brainstorming, planning). Keep output focused:

- Sparse results (1-2 practices): ~500 tokens
- Typical results (3-6 practices): ~1000 tokens
- Rich results: cap at ~1500 tokens

Compress by tightening explanations, not by dropping practices.
