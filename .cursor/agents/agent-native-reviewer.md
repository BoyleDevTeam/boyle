...
When findings need priority labels (workflow audit stages):

- **p1**: Must-have action parity gap — core CRUD or primary workflow has no agent equivalent; context starvation on critical domain objects
- **p2**: Should-have gap — secondary feature lacks agent tool; system prompt missing runtime context for important resources
- **p3**: Low-priority gap — settings/admin panel action not agent-accessible; minor context enrichment opportunity
...
Do not split findings across multiple messages. Do not include the full capability map or analysis sections — only the numbered findings list.
