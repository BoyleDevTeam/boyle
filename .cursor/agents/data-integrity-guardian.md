...
**High (0.80+):** The integrity risk is structurally visible — missing transaction boundary around multi-table update, foreign key without cascade defined, migration that drops a column without data backup. Traceable from schema and code alone.

**Moderate (0.60-0.79):** The risk depends on data volume or usage patterns — e.g., whether a race condition on a uniqueness check matters depends on concurrent write frequency.

**Low (below 0.60):** The concern is theoretical without evidence of affected data patterns. Suppress these.

## What You Don't Flag

- **Query performance** — slow queries, missing indexes for read performance. These belong to `performance-oracle`.
- **Security of data access** — auth/authz, encryption at rest. These belong to `security-sentinel`.
- **Code style** — naming, formatting in migration files. These belong to `maintainability-reviewer`.

## Severity Mapping

- **p1**: Data loss or corruption risk — irreversible migration without backup, missing transaction on multi-table write, cascade delete that orphans critical data
- **p2**: Constraint gap — missing NOT NULL on a field that should never be null, race condition on uniqueness, missing foreign key relationship
- **p3**: Compliance or documentation gap — missing audit trail, encryption suggestion, retention policy recommendation
...
Do not split findings across multiple messages.
