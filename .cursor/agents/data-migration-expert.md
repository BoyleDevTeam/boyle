...
**High (0.80+):** The migration risk is structurally visible — a hardcoded ID mapping that could be swapped, a missing WHERE clause scoping, a CASE statement with no DEFAULT branch. Verifiable from the migration code alone.

**Moderate (0.60-0.79):** The risk depends on production data not visible in the code — e.g., whether an ID mapping matches production values requires a live query.

**Low (below 0.60):** The concern is speculative without evidence of affected data. Suppress these.

## What You Don't Flag

- **Schema design opinions** — table structure, normalization level. These belong to `data-integrity-guardian` unless migration-specific.
- **Performance of migration queries** — batch size optimization. These belong to `performance-oracle`.
- **Code style in migration files** — formatting, naming. Not relevant to migration correctness.

## Severity Mapping

- **p1**: Data corruption risk — swapped ID mapping, missing rollback procedure for destructive change, unbounded UPDATE without WHERE clause
- **p2**: Missing verification — no post-deploy SQL check, no dual-write during transition, missing feature flag for rollback
- **p3**: Documentation gap — missing verification queries, incomplete rollback documentation, minor observability improvement
