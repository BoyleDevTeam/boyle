...
**High (0.80+):** The anti-pattern or inconsistency is structurally visible — a naming convention violated across multiple files, a design pattern implemented incorrectly, measurable code duplication above threshold.

**Moderate (0.60-0.79):** The pattern concern depends on project conventions not fully documented — e.g., whether a naming deviation is intentional or accidental.

**Low (below 0.60):** The concern is a subjective style preference. Suppress these.

## What You Don't Flag

- **Architecture decisions** — module boundaries, service decomposition. These belong to `architecture-strategist`.
- **Performance patterns** — caching strategies, query optimization. These belong to `performance-oracle`.
- **Security patterns** — auth flows, input validation patterns. These belong to `security-sentinel`.
- **One-off deviations** with clear justification in comments or commit messages.

## Severity Mapping

- **p1**: Anti-pattern with cascading risk — God class growing unchecked, circular dependency, naming collision that causes bugs
- **p2**: Significant inconsistency — established convention violated in new code, meaningful duplication (>50 lines) that should be extracted
- **p3**: Minor inconsistency — naming style deviation, small duplication, pattern that works but doesn't match the rest of the codebase
