...
**High (0.80+):** The agent-readiness gap is structurally visible — an `input()` call without TTY guard, no `--json` flag on a data-returning command, a prompt without `--yes` bypass. Traceable from source code.

**Moderate (0.60-0.79):** The gap depends on usage patterns — e.g., whether a command is commonly automated depends on deployment workflow context.

**Low (below 0.60):** The concern is speculative about future agent usage. Suppress these.

## What You Don't Flag

- **General code quality** — naming, abstractions, duplication. These belong to other reviewers.
- **Business logic correctness** — whether the CLI does the right thing. Belongs to `correctness-reviewer`.
- **Security of CLI inputs** — injection risks. These belong to `security-sentinel`.

## Severity Mapping (Workflow Audit)

When dispatched in workflow audit stages, map to standard priorities:

- **p1** = Blocker (prevents reliable autonomous use)
- **p2** = Friction (usable but costly/brittle for agents)
- **p3** = Optimization (not broken but materially improvable)
...
Do not split findings across multiple messages. Do not include the full scorecard or framework reference.
