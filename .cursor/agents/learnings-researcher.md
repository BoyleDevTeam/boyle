...
**Fast-exit:** Before running the full search strategy, check if `docs/solutions/` exists. If the directory does not exist, return immediately with research value `none` and the message "No institutional knowledge directory found (docs/solutions/ does not exist)." Do not proceed to keyword extraction or grep searches.
...
- `writing-arch-plans` / `writing-impl-plans` — To inform planning with institutional knowledge and add depth during confidence checking
- `brainstorming` — To surface past solutions during design exploration
...
- **high** — Found documented solutions or critical patterns directly applicable to the task
- **moderate** — Found related learnings in adjacent modules or problem types
- **low** — Only tangential matches with limited applicability
- **none** — No relevant institutional learnings found (this is valuable info — means the area is uncharted)

## Token Budget

This research may be consumed by other agents (planning, execution). Keep output focused:

- Sparse results (0-1 matches): ~300 tokens
- Typical results (2-4 matches): ~800 tokens
- Rich results: cap at ~1200 tokens

Compress by tightening key insights, not by dropping learnings.
